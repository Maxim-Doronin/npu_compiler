...
func.func @ConvAssignedSOHForLargeLayer(%arg0: tensor<1x64x608x608xf16, {order = #NHWC}>) -> tensor<1x80x608x608xf16, {order = #NHWC}> {
    ...
    %0 = VPU.NCE.Convolution(%arg0, %cst_0, %cst) {pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>, ppe = #VPU.PPEStub<>, rawFilterShape = [80, 64, 3, 3], strides = [1, 1]} : tensor<1x64x608x608xf16, {order = #NHWC}>, tensor<80x64x3x3xf16, {order = #NHWC}>, tensor<80x1x1x4xsi32> -> tensor<1x80x608x608xf16, {order = #NHWC}>
...
func.func @ConvAssignedSOKForSmallHW(%input_data: tensor<1x1024x4x4xf16, {order = #NHWC}>) -> tensor<1x2048x2x2xf16, {order = #NHWC}> {
    ...
    } : tensor<1x1024x4x4xf16, {order = #NHWC}>, tensor<2048x1024x1x1xf16, {order = #NHWC}>, tensor<2048x1x1x4xsi32> -> tensor<1x2048x2x2xf16, {order = #NHWC}>
...
func.func @ConvAssignedSOKInCaseOfINFCost(%arg0: tensor<1x4096x1x1xf16, {order = #NHWC}>) -> tensor<1x5504x1x1xf16, {order = #NHWC}> {
    ...
    } : tensor<1x4096x1x1xf16, {order = #NHWC}>, tensor<5504x4096x1x1x!qElemType, {order = #NHWC}>, tensor<5504x1x1x4xsi32> -> tensor<1x5504x1x1xf16, {order = #NHWC}>
...
func.func @SoftMaxAssignedSplitOverHeightAndBigConvolutionToo(%arg0: tensor<1x48x1024x4x!qElemType, {order = #NHWC}>) -> tensor<1x4096x1024x4xf16, {order = #NHWC}> {
    ...
    %0 = VPU.NCE.Convolution(%arg0, %cst, %cst_0) {ppe = #VPU.PPEStub<>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, rawFilterShape = [4096, 48, 1, 1], strides = [1, 1]} : tensor<1x48x1024x4x!qElemType, {order = #NHWC}>, tensor<4096x48x1x1x!qElemType, {order = #NHWC}>, tensor<4096x1x1x4xsi32> -> tensor<1x4096x1024x4xf16, {order = #NHWC}>
...
func.func @ConvAssignedSplitOverHeight(%arg0: tensor<1x1504x375x4xf16, {order = #NHWC}>) -> tensor<1x64x375x4xf16, {order = #NHWC}> {
    ...
    %1 = VPU.NCE.Convolution(%0, %cst, %cst_1) {
        pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
        ppe = #VPU.PPEStub<>, rawFilterShape = [64, 1504, 1, 1], strides = [1, 1]} : tensor<1x1504x375x4xf16, {order = #NHWC}>, tensor<64x1504x1x1xf16, {order = #NHWC}>, tensor<64x1x1x4xsi32> -> tensor<1x64x375x4xf16, {order = #NHWC}>
...
func.func @ThreeConvolutionsAssignedSOH(%input: tensor<1x128x92x92x!qElemType, {order = #NHWC}>) -> tensor<1x256x92x92x!qElemType, {order = #NHWC}> {
    ...
    %weights_table1 = const.Declare tensor<256x1x1x4xsi32> = dense<1> : tensor<256x1x1x4xsi32>
    %weights2 = const.Declare tensor<256x256x3x3x!qElemType, {order = #NHWC}> = dense<128> : tensor<256x256x3x3xui8, {order = #NHWC}>
    %weights_table2 = const.Declare tensor<256x1x1x4xsi32> = dense<1> : tensor<256x1x1x4xsi32>
    %weights3 = const.Declare tensor<256x256x3x3x!qElemType, {order = #NHWC}> = dense<128> : tensor<256x256x3x3xui8, {order = #NHWC}>
    %weights_table3 = const.Declare tensor<256x1x1x4xsi32> = dense<1> : tensor<256x1x1x4xsi32>

    %conv1 = VPU.NCE.Convolution(%input, %weights1, %weights_table1) {
        pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
        ppe = #VPU.PPEStub<>, rawFilterShape = [256, 128, 3, 3], strides = [1, 1]} :
            tensor<1x128x92x92x!qElemType, {order = #NHWC}>, tensor<256x128x3x3x!qElemType, {order = #NHWC}>, tensor<256x1x1x4xsi32> -> tensor<1x256x92x92x!qElemType, {order = #NHWC}>
    %conv2 = VPU.NCE.Convolution(%conv1, %weights2, %weights_table2) {
        pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
        ppe = #VPU.PPEStub<>, rawFilterShape = [256, 256, 3, 3], strides = [1, 1]} :
            tensor<1x256x92x92x!qElemType, {order = #NHWC}>, tensor<256x256x3x3x!qElemType, {order = #NHWC}>, tensor<256x1x1x4xsi32> -> tensor<1x256x92x92x!qElemType, {order = #NHWC}>
    %conv3 = VPU.NCE.Convolution(%conv2, %weights3, %weights_table3) {
        pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
        ppe = #VPU.PPEStub<>, rawFilterShape = [256, 256, 3, 3], strides = [1, 1]} :
            tensor<1x256x92x92x!qElemType, {order = #NHWC}>, tensor<256x256x3x3x!qElemType, {order = #NHWC}>, tensor<256x1x1x4xsi32> -> tensor<1x256x92x92x!qElemType, {order = #NHWC}>
