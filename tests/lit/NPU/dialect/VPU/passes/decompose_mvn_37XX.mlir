...
func.func @DecomposeMVNAcrossChannelFalseNHWC(%arg0: tensor<1x1x1x515971xf16, {order = #NHWC}>) -> (tensor<1x1x1x515971xf16, {order = #NHWC}>) {
    ...
    // CHECK:        [[MVN1SUM:%.+]] = VPU.MVN1SumOp([[SHAPECAST_1]]) {across_channels = false, normalize_variance = true, output_height = 2 : i64}
    ...
    // CHECK-SAME:       : tensor<1x1x2x2xf32, {order = #NHWC}> -> tensor<1x1x1x2xf16, {order = #NHWC}>
...
func.func @DecomposeMVNAcrossChannelTrueNHWC(%arg0: tensor<1x6x85995x1xf16, {order = #NHWC}>) -> (tensor<1x6x85995x1xf16, {order = #NHWC}>) {
    ...
    // CHECK:        [[MVN1SUM:%.+]] = VPU.MVN1SumOp([[SHAPECAST_1]]) {across_channels = true, normalize_variance = true, output_height = 2 : i64}
    // CHECK-SAME:       : tensor<1x1x515970x1xf16, {order = #NHWC}> -> tensor<1x1x2x2xf32, {order = #NHWC}>
...
func.func @DecomposeMVNAcrossChannelTrueNCHW(%arg0: tensor<1x5x1x115971xf16>) -> (tensor<1x5x1x115971xf16>) {
    ...
    // CHECK:        [[MVN1SUM:%.+]] = VPU.MVN1SumOp([[SHAPECAST_1]]) {across_channels = true, normalize_variance = true, output_height = 2 : i64}
    // CHECK-SAME:       : tensor<1x1x579855x1xf16> -> tensor<1x1x2x2xf32, {order = #NHWC}>
