...
func.func @NotBuildLargeWeights(%arg0: tensor<1x256x26x26xf16, {order = #NHWC}>) -> tensor<1x256x26x26xf16, {order = #NHWC}> {
    ...
    %0 = VPU.VerticalFusion (
      ...
      %2 = VPU.NCE.Convolution(%arg1, %arg2, %arg3)
         {pad =  #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
         ppe = #VPU.PPEStub<>,
         rawFilterShape = [512, 256, 3, 3], strides = [1, 1]} : tensor<1x256x26x26xf16, {order = #NHWC}>, tensor<512x256x3x3xf16, {order = #NHWC}>, tensor<512x1x1x4xsi32> -> tensor<1x512x26x26xf16, {order = #NHWC}>
      VPU.Yield %2
    }
    %1 = VPU.VerticalFusion (
      ...
      %2 = VPU.NCE.Convolution(%arg1, %arg2, %arg3)
         {pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
         ppe = #VPU.PPEStub<>,
         rawFilterShape = [256, 512, 3, 3], strides = [1, 1]} : tensor<1x512x26x26xf16, {order = #NHWC}>, tensor<256x512x3x3xf16, {order = #NHWC}>, tensor<256x1x1x4xsi32> -> tensor<1x256x26x26xf16, {order = #NHWC}>
      VPU.Yield %2
    }
