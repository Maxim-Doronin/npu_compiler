...
func.func @VfTilingWithEltwiseAdjustOffset(
    ...
    %0 = VPU.VerticalFusion (%arg0 as %arg1: tensor<1x16x128x128xf16, {order = #NHWC}>, %weights_1 as %arg2: tensor<32x16x3x3xf16, {order = #NHWC}>, %wt as %arg3: tensor<32x1x1x4xsi32>,
      ...
      %1 = VPU.NCE.Convolution(%arg1, %arg2, %arg3)
         ...
         pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
         ppe = #VPU.PPEStub<>,
         rawFilterShape = [32, 16, 3, 3], strides = [1, 1]} : tensor<1x16x128x128xf16, {order = #NHWC}>, tensor<32x16x3x3xf16, {order = #NHWC}>, tensor<32x1x1x4xsi32> -> tensor<1x32x128x128xf16, {order = #NHWC}>
      %2 = VPU.NCE.Convolution(%1, %arg4, %arg3)
         {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
         pad = #VPU.Padding<left = 3 : i64, right = 3 : i64, top = 3 : i64, bottom = 3 : i64>,
         ppe = #VPU.PPEStub<>,
         rawFilterShape = [32, 32, 7, 7], strides = [1, 1]} : tensor<1x32x128x128xf16, {order = #NHWC}>, tensor<32x32x7x7xf16, {order = #NHWC}>, tensor<32x1x1x4xsi32> -> tensor<1x32x128x128xf16, {order = #NHWC}>
      %3 = VPU.NCE.Eltwise(%1, %2)
         {is_inplace = true, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, op_type = #VPU.eltwise_type<ADD>,
         ppe = #VPU.PPEStub<>}
...
func.func @FallBackToOperandDueToSliceFail(%arg0: tensor<1x16x720x1280xf16, {order = #NHWC}>) -> tensor<1x16x720x1280xf16, {order = #NHWC}> {
    ...
    %0 = VPU.VerticalFusion (%arg0 as %arg1: tensor<1x16x720x1280xf16, {order = #NHWC}>, %cst as %arg2: tensor<16x16x3x3xf16, {order = #NHWC}>, %cst_0 as %arg3: tensor<16x1x1x4xsi32>) attributes {tilingStrategy = [1, 1, 1, 32]} -> tensor<1x16x720x1280xf16, {order = #NHWC}> {
      %1 = VPU.NCE.Convolution(%arg1, %arg2, %arg3) {mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>, rawFilterShape = [16, 16, 3, 3], strides = [2, 2]} : tensor<1x16x720x1280xf16, {order = #NHWC}>, tensor<16x16x3x3xf16, {order = #NHWC}>, tensor<16x1x1x4xsi32> -> tensor<1x16x359x639xf16, {order = #NHWC}>
    ...
    // CHECK-DAG: [[WT:%.+]] = const.Declare tensor<16x1x1x4xsi32>

    // CHECK: [[CONV_SLICE_0:%.+]] = VPU.Slice [[INPUT]] [0, 0, 0, 0] [1, 16, 720, 57]
    // CHECK: [[CONV_0:%.+]] = VPU.NCE.Convolution([[CONV_SLICE_0]], [[WEIGHTS]], [[WT]])
    ...
    // CHECK: [[CONV_1:%.+]] = VPU.NCE.Convolution([[CONV_SLICE_1]], [[WEIGHTS]], [[WT]])
    ...
    // CHECK: [[CONV_30:%.+]] = VPU.NCE.Convolution([[CONV_SLICE_30]], [[WEIGHTS]], [[WT]])
    ...
    // CHECK: [[CONV_31:%.+]] = VPU.NCE.Convolution([[CONV_SLICE_31]], [[WEIGHTS]], [[WT]])
...
func.func @VfTilingWithMultiEltwiseAdjustOffset(
    ...
    %0 = VPU.VerticalFusion (%arg0 as %arg1: tensor<1x16x128x128xf16, {order = #NHWC}>, %weights_1 as %arg2: tensor<32x16x3x3xf16, {order = #NHWC}>, %wt as %arg3: tensor<32x1x1x4xsi32>,
      ...
      %3 = VPU.NCE.Eltwise(%1, %2)
         ...
         ppe = #VPU.PPEStub<>}
         -> tensor<1x32x128x128xf16, {order = #NHWC}>
      %4 = VPU.NCE.Convolution(%3, %arg4, %arg3)
         {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
         pad = #VPU.Padding<left = 3 : i64, right = 3 : i64, top = 3 : i64, bottom = 3 : i64>,
         ppe = #VPU.PPEStub<>,
         rawFilterShape = [32, 32, 7, 7], strides = [1, 1]} : tensor<1x32x128x128xf16, {order = #NHWC}>, tensor<32x32x7x7xf16, {order = #NHWC}>, tensor<32x1x1x4xsi32> -> tensor<1x32x128x128xf16, {order = #NHWC}>
