...
func.func @EfficientEltwiseOrder(%arg0: tensor<1x96x24x40xf16, {order = #NHWC}>) -> tensor<1x96x24x40xf16, {order = #NHWC}> {
    ...
    %0 = VPU.NCE.Convolution(%arg0, %cst_1, %cst) {ppe = #VPU.PPEStub<>, pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>, rawFilterShape = [96, 96, 3, 3], strides = [1, 1]} : tensor<1x96x24x40xf16, {order = #NHWC}>, tensor<96x96x3x3xf16, {order = #NHWC}>, tensor<96x1x1x4xsi32> -> tensor<1x96x24x40xf16, {order = #NHWC}>
    %1 = VPU.NCE.Convolution(%0, %cst_1, %cst) {ppe = #VPU.PPEStub<>, pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>, rawFilterShape = [96, 96, 3, 3], strides = [1, 1]} : tensor<1x96x24x40xf16, {order = #NHWC}>, tensor<96x96x3x3xf16, {order = #NHWC}>, tensor<96x1x1x4xsi32> -> tensor<1x96x24x40xf16, {order = #NHWC}>
    %2 = VPU.NCE.Convolution(%1, %cst_1, %cst) {ppe = #VPU.PPEStub<>, pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>, rawFilterShape = [96, 96, 3, 3], strides = [1, 1]} : tensor<1x96x24x40xf16, {order = #NHWC}>, tensor<96x96x3x3xf16, {order = #NHWC}>, tensor<96x1x1x4xsi32> -> tensor<1x96x24x40xf16, {order = #NHWC}>
    %3 = VPU.NCE.Convolution(%0, %cst_1, %cst) {ppe = #VPU.PPEStub<>, pad = #VPU.Padding<left = 0 : i64, right = 1 : i64, top = 0 : i64, bottom = 1 : i64>, rawFilterShape = [96, 96, 3, 3], strides = [2, 2]} : tensor<1x96x24x40xf16, {order = #NHWC}>, tensor<96x96x3x3xf16, {order = #NHWC}>, tensor<96x1x1x4xsi32> -> tensor<1x96x12x20xf16, {order = #NHWC}>
...
func.func @EfficientGPTQOperationsOrder(
    ...
    %0 = VPU.VerticalFusion (
        ...
        } : tensor<1x128x256x4xf16, {order = #NHWC}>, tensor<3584x128x1x1x!qElemType, {order = #NHWC}>, tensor<3584x1x1x4xsi32> -> tensor<1x3584x256x4xf16, {order = #NHWC}>
...
func.func @NotReorderOpsOutsideVFBlock(%arg0: tensor<1x64x30x30xf16, {order = #NHWC}>, %arg1: tensor<1x64x30x30xf16, {order = #NHWC}>) -> tensor<1x64x15x15x!qElemType2, {order = #NHWC}> {
    ...
    %1 = VPU.NCE.Convolution(%0, %cst_2, %cst_3) {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, ppe = #VPU.PPEStub<>, pad = #VPU.Padding<left = 1 : i64, right = 0 : i64, top = 1 : i64, bottom = 0 : i64>, rawFilterShape = [32, 64, 3, 3], strides = [2, 2]} : tensor<1x64x30x30x!qElemType1, {order = #NHWC}>, tensor<32x64x3x3x!qElemType, {order = #NHWC}>, tensor<32x1x1x4xsi32> -> tensor<1x32x15x15x!qElemType2, {order = #NHWC}>
    ...
    %3 = VPU.NCE.Convolution(%2, %cst_6, %cst_7) {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, ppe = #VPU.PPEStub<>, pad = #VPU.Padding<left = 1 : i64, right = 0 : i64, top = 1 : i64, bottom = 0 : i64>, rawFilterShape = [32, 64, 3, 3], strides = [2, 2]} : tensor<1x64x30x30x!qElemType1, {order = #NHWC}>, tensor<32x64x3x3x!qElemType, {order = #NHWC}>, tensor<32x1x1x4xsi32> -> tensor<1x32x15x15x!qElemType2, {order = #NHWC}>
