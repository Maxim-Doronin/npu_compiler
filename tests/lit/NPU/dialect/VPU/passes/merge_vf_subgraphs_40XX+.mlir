...
func.func @BuildSubgraphConvSwishGroupConvVF(%arg0: tensor<1x16x176x176xf16, {order = #NHWC}>) -> tensor<1x96x176x176xf16, {order = #NHWC}> {
    ...
    //CHECK:      [[CONV0:%.+]] = VPU.NCE.Convolution(%arg1, %arg2, %arg3) {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>, rawFilterShape = [96, 16, 1, 1], strides = [1, 1]}
...
func.func @BuildSubgraphVFInput(%arg0: tensor<1x16x256x256x!qElemType, {order = #NHWC}>, %arg1: tensor<1x16x256x256x!qElemType2, {order = #NHWC}>) -> (tensor<1x32x256x256x!qElemType, {order = #NHWC}>) {
    ...
    %0 = VPU.VerticalFusion (%arg1 as %arg2: tensor<1x16x256x256x!qElemType2, {order = #NHWC}>, %cst_0 as %arg3: tensor<32x16x3x3x!qElemType1, {order = #NHWC}>, %cst_1 as %arg4: tensor<32x1x1x4xsi32>) attributes {tilingStrategy = [1, 1, 1, 1]} -> tensor<1x32x256x256x!qElemType2, {order = #NHWC}> {
      %4 = VPU.NCE.Convolution(%arg2, %arg3, %arg4)
         ...
         pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
         ppe = #VPU.PPEStub<>,
         rawFilterShape = [32, 16, 3, 3], strides = [1, 1]} : tensor<1x16x256x256x!qElemType2, {order = #NHWC}>, tensor<32x16x3x3x!qElemType1, {order = #NHWC}>, tensor<32x1x1x4xsi32> -> tensor<1x32x256x256x!qElemType2, {order = #NHWC}>
      VPU.Yield %4
    }
...
func.func @BuildSubgraphConvGeluVF(%arg0: tensor<1x16x128x128xf16, {order = #NHWC}>) -> tensor<1x64x128x128xf16, {order = #NHWC}> {
    ...
    %0 = VPU.VerticalFusion (%arg0 as %arg2: tensor<1x16x128x128xf16, {order = #NHWC}>, %cst_0 as %arg3: tensor<64x16x1x1xf16, {order = #NHWC}>, %cst_1 as %arg4: tensor<64x1x1x4xsi32>) attributes {tilingStrategy = [1, 1, 2, 1]} -> tensor<1x64x128x128xf16, {order = #NHWC}> {
      %3 = VPU.NCE.Convolution(%arg2, %arg3, %arg4)
        {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>, rawFilterShape = [64, 16, 1, 1], strides = [1, 1]} : tensor<1x16x128x128xf16, {order = #NHWC}>, tensor<64x16x1x1xf16, {order = #NHWC}>, tensor<64x1x1x4xsi32> -> tensor<1x64x128x128xf16, {order = #NHWC}>
      VPU.Yield %3
    }
    ...
    //CHECK: [[NCE:%.+]] = VPU.NCE.Convolution([[INNER_ARG1]], [[INNER_ARG2]], [[INNER_ARG3]]) {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>, rawFilterShape = [64, 16, 1, 1], strides = [1, 1]}
...
func.func @NotBuildOutOfLimitNumTiles(%arg0: tensor<1x512x92x120x!qElemType, {order = #NHWC}>) -> tensor<1x256x92x120x!qElemType, {order = #NHWC}> {
    ...
    %0 = VPU.VerticalFusion (%arg0 as %arg1: tensor<1x512x92x120x!qElemType, {order = #NHWC}>, %cst_0 as %arg2: tensor<256x512x3x3x!qElemType, {order = #NHWC}>, %cst_1 as %arg3: tensor<256x1x1x4xsi32>) attributes {tilingStrategy = [1, 1, 92, 1]} -> tensor<1x256x92x120x!qElemType, {order = #NHWC}> {
      %2 = VPU.NCE.Convolution(%arg1, %arg2, %arg3)
         {pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
          ppe = #VPU.PPEStub<>,
          rawFilterShape = [256, 512, 3, 3], strides = [1, 1]} : tensor<1x512x92x120x!qElemType, {order = #NHWC}>, tensor<256x512x3x3x!qElemType, {order = #NHWC}>, tensor<256x1x1x4xsi32> -> tensor<1x256x92x120x!qElemType, {order = #NHWC}>
      VPU.Yield %2
    }
    ...
    %1 = VPU.VerticalFusion (%0 as %arg1: tensor<1x256x92x120x!qElemType, {order = #NHWC}>, %cst_2 as %arg2: tensor<256x256x3x3x!qElemType, {order = #NHWC}>, %cst_1 as %arg3: tensor<256x1x1x4xsi32>) attributes {tilingStrategy = [1, 1, 16, 1]} -> tensor<1x256x92x120x!qElemType, {order = #NHWC}> {
      %2 = VPU.NCE.Convolution(%arg1, %arg2, %arg3)
          {pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
           ppe = #VPU.PPEStub<>,
           rawFilterShape = [256, 256, 3, 3], strides = [1, 1]} : tensor<1x256x92x120x!qElemType, {order = #NHWC}>, tensor<256x256x3x3x!qElemType, {order = #NHWC}>, tensor<256x1x1x4xsi32> -> tensor<1x256x92x120x!qElemType, {order = #NHWC}>
      VPU.Yield %2
    }
...
func.func @PartialBuildWithWeightsConstraints(%arg0: tensor<1x256x48x14xf16, {order = #NHWC}>) -> tensor<1x256x48x14xf16, {order = #NHWC}> {
    ...
    %0 = VPU.VerticalFusion (
      ...
      %3 = VPU.NCE.Convolution(%arg1, %arg2, %arg3)
         {pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
         rawFilterShape = [256, 256, 3, 3], ppe = #VPU.PPEStub<>, strides = [1, 1]} : tensor<1x256x48x14xf16, {order = #NHWC}>, tensor<256x256x3x3xf16, {order = #NHWC}>, tensor<256x1x1x4xsi32> -> tensor<1x256x48x14xf16, {order = #NHWC}>
      VPU.Yield %3
    }
    %1 = VPU.VerticalFusion (
      ...
      %3 = VPU.NCE.Convolution(%arg1, %arg2, %arg3)
         {pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
         rawFilterShape = [256, 256, 1, 1], ppe = #VPU.PPEStub<>, strides = [1, 1]} : tensor<1x256x48x14xf16, {order = #NHWC}>, tensor<256x256x1x1xf16, {order = #NHWC}>, tensor<256x1x1x4xsi32> -> tensor<1x256x48x14xf16, {order = #NHWC}>
      VPU.Yield %3
    }
    %2 = VPU.VerticalFusion (
...
func.func @BuildSubgraphConvWithDepthToSpace(%arg0: tensor<1x16x180x270xf16, {order = #NHWC}>) -> tensor<1x1x720x1080xf16, {order = #NHWC}> {
    ...
    %0 = VPU.VerticalFusion (%arg0 as %arg1: tensor<1x16x180x270xf16, {order = #NHWC}>, %cst_0 as %arg2: tensor<16x16x5x5xf16, {order = #NHWC}>, %cst_1 as %arg3: tensor<16x1x1x4xsi32>) attributes {tilingStrategy = [1, 1, 4, 1]} -> tensor<1x16x180x270xf16, {order = #NHWC}> {
      ...
      ppe = #VPU.PPEStub<>,
      rawFilterShape = [16, 16, 5, 5], strides = [1, 1]} : tensor<1x16x180x270xf16, {order = #NHWC}>, tensor<16x16x5x5xf16, {order = #NHWC}>, tensor<16x1x1x4xsi32> -> tensor<1x16x180x270xf16, {order = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>}>
      VPU.Yield %3
    }
...
func.func @BuildSubgraphConvPReluVF(%arg0: tensor<1x16x128x128xf16, {order = #NHWC}>) -> tensor<1x16x128x128xf16, {order = #NHWC}> {
    ...
    %0 = VPU.VerticalFusion (%arg0 as %arg2: tensor<1x16x128x128xf16, {order = #NHWC}>, %cst_0 as %arg3: tensor<16x16x1x1xf16, {order = #NHWC}>, %cst_1 as %arg4: tensor<16x1x1x4xsi32>) attributes {tilingStrategy = [1, 1, 2, 1]} -> tensor<1x16x128x128xf16, {order = #NHWC}> {
      %3 = VPU.NCE.Convolution(%arg2, %arg3, %arg4)
        {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>, rawFilterShape = [16, 16, 1, 1], strides = [1, 1]} : tensor<1x16x128x128xf16, {order = #NHWC}>, tensor<16x16x1x1xf16, {order = #NHWC}>, tensor<16x1x1x4xsi32> -> tensor<1x16x128x128xf16, {order = #NHWC}>
      VPU.Yield %3
    }
    ...
    // CHECK: [[NCE:%.+]] = VPU.NCE.Convolution([[INNER_ARG1]], [[INNER_ARG2]], [[INNER_ARG3]]) {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>, rawFilterShape = [16, 16, 1, 1], strides = [1, 1]}
...
func.func @BuildSubgraphMemPermuteClustered(%arg0: tensor<1x16x256x256xf16, {order = #NWCH}>) -> tensor<1x32x256x256xf16, {order = #NHWC}> {
    ...
    %1 = VPU.VerticalFusion (%0 as %arg1: tensor<1x16x256x256xf16, {order = #NHWC}>, %cst_0 as %arg2: tensor<32x16x1x1xf16, {order = #NHWC}>, %cst_1 as %arg3: tensor<32x1x1x4xsi32>) attributes {tilingStrategy = [1, 1, 4, 1]} -> tensor<1x32x256x256xf16, {order = #NHWC}> {
      %3 = VPU.NCE.Convolution(%arg1, %arg2, %arg3)
        {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
        pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
        ppe = #VPU.PPEStub<>,
        rawFilterShape = [32, 16, 1, 1], strides = [1, 1]} : tensor<1x16x256x256xf16, {order = #NHWC}>, tensor<32x16x1x1xf16, {order = #NHWC}>, tensor<32x1x1x4xsi32> -> tensor<1x32x256x256xf16, {order = #NHWC}>
      VPU.Yield %3
    }
    ...
    // CHECK:      [[CONV:%.+]] = VPU.NCE.Convolution([[PERMUTE]], %arg2, %arg3)
