...
!ConvWeightsTableTensor = !VPU.DistributedTensor<256x1x1x4xsi32, #NCHW, @CMX_NN, {
    mode = "DUPLICATED", num_clusters = 6 : i64, uniform_distributed_segments,
...
// CHECK-SAME:      [[CONV_WEIGHTS_TABLE:%.+]]: !VPU.DistributedTensor<256x1x1x4xsi32, #NCHW, @CMX_NN,
func.func @OptimizeShapeCastCopies(%conv_input: !ConvInputTensor, %conv_weights: !ConvWeightsTensor, %conv_weights_table: !ConvWeightsTableTensor) -> !NextOverlappingInputTensor {
    %conv = VPU.NCE.Convolution(%conv_input, %conv_weights, %conv_weights_table) {
        pad = #VPU.Padding<left = 0 : i64, right = 1 : i64, top = 0 : i64, bottom = 1 : i64>,
        ppe = #VPU.PPEStub<>,
        rawFilterShape = [256, 48, 3, 2],
        strides = [2, 1]
    } : !ConvInputTensor, !ConvWeightsTensor, !ConvWeightsTableTensor -> !ConvOutputTensor
    ...
    // CHECK:               [[CONV:%.+]] = VPU.NCE.Convolution([[CONV_INPUT]], [[CONV_WEIGHTS]], [[CONV_WEIGHTS_TABLE]]) {
