...
// RUN: vpux-opt --split-input-file --init-compiler="vpu-arch=%arch% allow-custom-values=true" --move-tensor-ops-to-cmx %s | FileCheck %s
...
!convInTiledDDRType = tensor<1x32x?x66xf16, {bounds = #const.OpaqueI64Elements<[1, 32, 66, 66]> : tensor<4xsi64>, order = #NHWC}>
...
func.func @SOHConvTileOverH(%arg0: tensor<1x32x64x64xf16, {order = #NHWC}>) -> tensor<1x256x64x64xf16, {order = #NHWC}> {
  ...
  %1 = scf.for %arg1 = %c0 to %c64 step %c32 iter_args(%arg2 = %0) -> (tensor<1x256x64x64xf16, {order = #NHWC}>) {
    ...
    ^bb0(%arg3: index, %arg4: index, %arg5: index, %arg6: index):
      tensor.yield %cst : f16
    } : tensor<1x32x33x64xf16, {order = #NHWC}> to !convInTiledDDRType

    %copy_act = VPU.Copy(%padded) {out_mem_space = @CMX_NN}
      : !convInTiledDDRType -> !convInTiledType
    ...
    %dim = tensor.dim %padded, %c2 : !convInTiledDDRType
    ...
    %10 = scf.forall (%arg3) = (0) to (%7) step (%9) shared_outs(%arg4 = %8) -> (!convOutTiledType) {
      ...
      %extracted_slice_1 = tensor.extract_slice %copy_act[0, 0, %arg3, 0] [1, 32, %12, 66] [1, 1, 1, 1]
    ...
    %copy_output = VPU.Copy(%10) {out_mem_space = @DDR}
      : !convOutTiledType
...
// CHECK:       [[IN_COPY:%.+]] = VPU.Copy([[IN_SLICE]]) {out_mem_space = @CMX_NN}
...
// CHECK:         [[IN_MC0:%.+]] = tensor.extract_slice [[IN_PAD]]
