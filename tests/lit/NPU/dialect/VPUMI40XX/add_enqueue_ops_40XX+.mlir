...
// RUN: vpux-opt --split-input-file --init-compiler="vpu-arch=%arch% allow-custom-values=true" --add-enqueue-ops="enable-predefined-enqueue=false" %s | FileCheck %s
...
module @Convolution attributes {config.compilationMode = #config.compilation_mode<DefaultHW>} {
  ...
  func.func @main(%arg0: memref<1x16x16x16xf16, @DDR>, %arg1: memref<1x16x14x14xf16, @DDR>) -> memref<1x16x14x14xf16, @DDR> {
    ...
    taskLocation(%16 : !VPURegMapped.Index<0:0:1>) previousTask(%19 : !VPURegMapped.Index<0:0:0>) input(%7 : memref<1x16x16x16xf16, #NHWC, [@CMX_NN, 0]>) weights(%9 : memref<16x16x3x3xf16, #NHWC, [@CMX_NN, 0]>)
...
//CHECK: [[VAL30:%.+]] = VPURegMapped.Enqueue at([[VAL10]] : !VPURegMapped.Index<0:0:0>) ([[VAL19]] -> [[VAL19]] : <0:0:0> -> <0:0:0>) -> !VPURegMapped.Index<0:0:0> {taskType = #VPURegMapped.task_type<DPUVariant>}
//CHECK: [[VAL31:%.+]] = VPURegMapped.Enqueue previousTaskIdx([[VAL30]] : !VPURegMapped.Index<0:0:0>) at([[VAL10]] : !VPURegMapped.Index<0:0:0>) ([[VAL28]] -> [[VAL28]] : <0:0:3> -> <0:0:3>) -> !VPURegMapped.Index<0:0:1> {taskType = #VPURegMapped.task_type<DMA>}
