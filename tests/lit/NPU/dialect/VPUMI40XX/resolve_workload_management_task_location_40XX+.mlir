...
// CHECK: [[TBI30:%.*]] = VPUMI40XX.DeclareTaskBuffer <DPUInvariant> -> !VPURegMapped.Index<0:0:30>
// CHECK: [[TBI31:%.*]] = VPUMI40XX.DeclareTaskBuffer <DPUInvariant> -> !VPURegMapped.Index<0:0:31>

// CHECK: [[TBV62:%.*]] = VPUMI40XX.DeclareTaskBuffer <DPUVariant> -> !VPURegMapped.Index<0:0:62>
// CHECK: [[TBV63:%.*]] = VPUMI40XX.DeclareTaskBuffer <DPUVariant> -> !VPURegMapped.Index<0:0:63>
...
module @TestSoftmax {
  ...
  %49 = VPURegMapped.FetchTask primary(%startIndexes_0#0 -> %endIndexes_1#0) secondary(%startIndexes_0#1 -> %endIndexes_1#1) (<1:0:0> -> <1:0:3> : !VPURegMapped.Index<1:0:0> -> !VPURegMapped.Index<1:0:3>) -> <0:0:0> {associated_execution_group_index = 0 : ui64, associated_task_type = #VPURegMapped.task_type<ActKernelRange>, associated_tile_index = 1 : ui64}
  %50 = VPURegMapped.FetchTask previousTask(%49 : !VPURegMapped.Index<0:0:0>) primary(%startIndexes#0 -> %endIndexes#0) secondary(%startIndexes#1 -> %endIndexes#1) (<0:0:0> -> <0:0:3> : !VPURegMapped.Index<0:0:0> -> !VPURegMapped.Index<0:0:3>) -> <0:0:1> {associated_execution_group_index = 0 : ui64, associated_task_type = #VPURegMapped.task_type<ActKernelRange>, associated_tile_index = 0 : ui64}
  %51 = VPUMI40XX.NNDMA {dma_descriptor = #VPUIP.DMADescriptorAttr<numPlanes = 0 : i64, len = 0 : i64, srcWidth = 0 : i64, srcStride = 0 : i64, srcPlaneStride = 0 : i64, dstWidth = 0 : i64, dstStride = 0 : i64, dstPlaneStride = 0 : i64>, port = 0 : i64} inputs(%2 : memref<0x0x0x0xi32, @DDR>) outputs(%3 : memref<0x0x0x0xi32, @DDR>) previousDMA(%50 : !VPURegMapped.Index<0:0:1>) updates(%42 : !VPURegMapped.Index<0:0:0>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>) -> !VPURegMapped.Index<0:0:2>
  %52 = VPUMI40XX.NNDMA {is_out_of_order, port = 0 : i64} inputs(%0 : memref<1x1000x1x1xf16, affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>, @DDR>) outputs(%22, %23 : memref<1x1000x1x1xf16, affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>, [@CMX_NN, 0]>, memref<1x1000x1x1xf16, affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>, [@CMX_NN, 1]>) previousDMA(%51 : !VPURegMapped.Index<0:0:2>) waits(%42 : !VPURegMapped.Index<0:0:0>) updates(%43 : !VPURegMapped.Index<0:0:1>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>) dma_transaction(#VPUMI40XX.NNDMATransaction<inputType = memref<1x1000x1x1xf16, affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>, @DDR>, outputType = !VPUIP.DistributedBuffer<1x1000x1x1xf16, affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64, uniform_distributed_segments, compute_shapes = [[1, 1000, 1, 1], [1, 1000, 1, 1]], compute_offsets = [[0, 0, 0, 0], [0, 0, 0, 0]], memory_shapes = [[1, 1000, 1, 1], [1, 1000, 1, 1]], memory_offsets = [[0, 0, 0, 0], [0, 0, 0, 0]]}>>) -> !VPURegMapped.Index<0:0:3>
  ...
  %56 = VPUMI40XX.MappedInference dmas((%49, %53) : (!VPURegMapped.Index<0:0:0>, !VPURegMapped.Index<0:1:0>)) actKernelRanges(%startIndexes#0, %startIndexes_0#0 : !VPURegMapped.Index<0:0:0>, !VPURegMapped.Index<1:0:0>) actKernelInvocations(%startIndexes#1, %startIndexes_0#1 : !VPURegMapped.Index<0:0:0>, !VPURegMapped.Index<1:0:0>) barriers(%42 : !VPURegMapped.Index<0:0:0>) actShaveRt(%55 : !VPURegMapped.Index<0:0:0>) dmaHwpBase(%4 : memref<16xui32, [@CMX_NN, 0]>) dmaCount([[4, 1], [0, 0]]) invariantCount([0, 0]) variantCount([0, 0]) actKernelRangesCount([4, 4]) actKernelInvocationsCount([4, 4]) mediaCount(0) barrierCount(7) -> !VPURegMapped.Index<0:0:0>
  return %arg1 : memref<1x1000x1x1xf16, @DDR>
  }
}

// CHECK: [[TBI77:%.+]] = VPUMI40XX.DeclareTaskBuffer <ActKernelInvocation> -> !VPURegMapped.Index<0:0:28>
// CHECK: [[TBI78:%.+]] = VPUMI40XX.DeclareTaskBuffer <ActKernelInvocation> -> !VPURegMapped.Index<0:0:29>
// CHECK: [[TBI79:%.+]] = VPUMI40XX.DeclareTaskBuffer <ActKernelInvocation> -> !VPURegMapped.Index<0:0:30>
// CHECK: [[TBI80:%.+]] = VPUMI40XX.DeclareTaskBuffer <ActKernelInvocation> -> !VPURegMapped.Index<0:0:31>

// CHECK: [[TBR141:%.+]] = VPUMI40XX.DeclareTaskBuffer <ActKernelRange> -> !VPURegMapped.Index<0:0:28>
// CHECK: [[TBR142:%.+]] = VPUMI40XX.DeclareTaskBuffer <ActKernelRange> -> !VPURegMapped.Index<0:0:29>
// CHECK: [[TBR143:%.+]] = VPUMI40XX.DeclareTaskBuffer <ActKernelRange> -> !VPURegMapped.Index<0:0:30>
// CHECK: [[TBR144:%.+]] = VPUMI40XX.DeclareTaskBuffer <ActKernelRange> -> !VPURegMapped.Index<0:0:31>

//CHECK: VPUMI40XX.ActKernelRange
//CHECK-SAME: taskLocation([[TBR141]] : !VPURegMapped.Index<0:0:28>)

//CHECK: VPUMI40XX.ActKernelRange
//CHECK-SAME: taskLocation([[TBR142]] : !VPURegMapped.Index<0:0:29>)

//CHECK: VPUMI40XX.ActKernelRange
//CHECK-SAME: taskLocation([[TBR143]] : !VPURegMapped.Index<0:0:30>)

//CHECK: VPUMI40XX.ActKernelRange
//CHECK-SAME: taskLocation([[TBR144]] : !VPURegMapped.Index<0:0:31>)

//CHECK: VPUMI40XX.ActKernelInvocation
//CHECK-SAME: taskLocation([[TBI77]] : !VPURegMapped.Index<0:0:28>)

//CHECK: VPUMI40XX.ActKernelInvocation
//CHECK-SAME: taskLocation([[TBI78]] : !VPURegMapped.Index<0:0:29>)

//CHECK: VPUMI40XX.ActKernelInvocation
//CHECK-SAME: taskLocation([[TBI79]] : !VPURegMapped.Index<0:0:30>)

//CHECK: VPUMI40XX.ActKernelInvocation
//CHECK-SAME: taskLocation([[TBI80]] : !VPURegMapped.Index<0:0:31>)

// CHECK: [[TBI205:%.+]] = VPUMI40XX.DeclareTaskBuffer <ActKernelInvocation> -> !VPURegMapped.Index<1:0:28>
// CHECK: [[TBI206:%.+]] = VPUMI40XX.DeclareTaskBuffer <ActKernelInvocation> -> !VPURegMapped.Index<1:0:29>
// CHECK: [[TBI207:%.+]] = VPUMI40XX.DeclareTaskBuffer <ActKernelInvocation> -> !VPURegMapped.Index<1:0:30>
// CHECK: [[TBI208:%.+]] = VPUMI40XX.DeclareTaskBuffer <ActKernelInvocation> -> !VPURegMapped.Index<1:0:31>

// CHECK: [[TBR269:%.+]] = VPUMI40XX.DeclareTaskBuffer <ActKernelRange> -> !VPURegMapped.Index<1:0:28>
// CHECK: [[TBR270:%.+]] = VPUMI40XX.DeclareTaskBuffer <ActKernelRange> -> !VPURegMapped.Index<1:0:29>
// CHECK: [[TBR271:%.+]] = VPUMI40XX.DeclareTaskBuffer <ActKernelRange> -> !VPURegMapped.Index<1:0:30>
// CHECK: [[TBR272:%.+]] = VPUMI40XX.DeclareTaskBuffer <ActKernelRange> -> !VPURegMapped.Index<1:0:31>

//CHECK: VPUMI40XX.ActKernelRange
//CHECK-SAME: taskLocation([[TBR269]] : !VPURegMapped.Index<1:0:28>)

//CHECK: VPUMI40XX.ActKernelRange
//CHECK-SAME: taskLocation([[TBR270]] : !VPURegMapped.Index<1:0:29>)

//CHECK: VPUMI40XX.ActKernelRange
//CHECK-SAME: taskLocation([[TBR271]] : !VPURegMapped.Index<1:0:30>)

//CHECK: VPUMI40XX.ActKernelRange
//CHECK-SAME: taskLocation([[TBR272]] : !VPURegMapped.Index<1:0:31>)

//CHECK: VPUMI40XX.ActKernelInvocation
//CHECK-SAME: taskLocation([[TBI205]] : !VPURegMapped.Index<1:0:28>)

//CHECK: VPUMI40XX.ActKernelInvocation
//CHECK-SAME: taskLocation([[TBI206]] : !VPURegMapped.Index<1:0:29>)

//CHECK: VPUMI40XX.ActKernelInvocation
//CHECK-SAME: taskLocation([[TBI207]] : !VPURegMapped.Index<1:0:30>)

//CHECK: VPUMI40XX.ActKernelInvocation
//CHECK-SAME: taskLocation([[TBI208]] : !VPURegMapped.Index<1:0:31>)
