...
#NHWC = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>
#NWCH = affine_map<(d0, d1, d2, d3) -> (d0, d3, d1, d2)>
module @Softmax attributes {VPU.compilationMode = #VPU.compilation_mode<DefaultHW>} {
  IE.TileResource 2 of @NCE at 1.700000e+03 MHz {
    builtin.module @ReservedMemory {
      module @DmaProfilingReservedMemory {
        IE.MemoryResource 512 bytes of @CMX_NN offset 0
  ...
  %181 = VPUMI40XX.ActKernelInvocation taskLocation(%77 : !VPURegMapped.Index<0:0:28>) range_index(%177 : <0:0:0>) kernel_params(%34 : <0:0:0>) waits(%43 : !VPURegMapped.Index<0:0:1>) updates(%44 : !VPURegMapped.Index<0:0:2>) enqueueBarrier(%42 : !VPURegMapped.Index<0:0:0>) tile(0) start_after(0) clean_after(0) -> !VPURegMapped.Index<0:0:0>
  %182 = VPUMI40XX.ActKernelInvocation taskLocation(%78 : !VPURegMapped.Index<0:0:29>) previousTask(%181 : !VPURegMapped.Index<0:0:0>) range_index(%178 : <0:0:1>) kernel_params(%36 : <0:0:1>) waits(%44 : !VPURegMapped.Index<0:0:2>) updates(%45 : !VPURegMapped.Index<0:0:3>) enqueueBarrier(%42 : !VPURegMapped.Index<0:0:0>) tile(0) start_after(0) clean_after(0) -> !VPURegMapped.Index<0:0:1>
  %183 = VPUMI40XX.ActKernelInvocation taskLocation(%79 : !VPURegMapped.Index<0:0:30>) previousTask(%182 : !VPURegMapped.Index<0:0:1>) range_index(%179 : <0:0:2>) kernel_params(%38 : <0:0:2>) waits(%45 : !VPURegMapped.Index<0:0:3>) updates(%46 : !VPURegMapped.Index<0:0:4>) enqueueBarrier(%42 : !VPURegMapped.Index<0:0:0>) tile(0) start_after(0) clean_after(0) -> !VPURegMapped.Index<0:0:2>
  ...
  %317 = VPUMI40XX.ActKernelInvocation taskLocation(%213 : !VPURegMapped.Index<1:0:28>) range_index(%313 : <1:0:0>) kernel_params(%35 : <1:0:0>) waits(%43 : !VPURegMapped.Index<0:0:1>) updates(%44 : !VPURegMapped.Index<0:0:2>) enqueueBarrier(%42 : !VPURegMapped.Index<0:0:0>) tile(1) start_after(0) clean_after(0) -> !VPURegMapped.Index<1:0:0>
  %318 = VPUMI40XX.ActKernelInvocation taskLocation(%214 : !VPURegMapped.Index<1:0:29>) previousTask(%317 : !VPURegMapped.Index<1:0:0>) range_index(%314 : <1:0:1>) kernel_params(%37 : <1:0:1>) waits(%44 : !VPURegMapped.Index<0:0:2>) updates(%45 : !VPURegMapped.Index<0:0:3>) enqueueBarrier(%42 : !VPURegMapped.Index<0:0:0>) tile(1) start_after(0) clean_after(0) -> !VPURegMapped.Index<1:0:1>
  %319 = VPUMI40XX.ActKernelInvocation taskLocation(%215 : !VPURegMapped.Index<1:0:30>) previousTask(%318 : !VPURegMapped.Index<1:0:1>) range_index(%315 : <1:0:2>) kernel_params(%39 : <1:0:2>) waits(%45 : !VPURegMapped.Index<0:0:3>) updates(%46 : !VPURegMapped.Index<0:0:4>) enqueueBarrier(%42 : !VPURegMapped.Index<0:0:0>) tile(1) start_after(0) clean_after(0) -> !VPURegMapped.Index<1:0:2>
  ...
  %328 = VPUMI40XX.MappedInference {workloadManagementBarrierProgrammingMode = #VPURegMapped.workload_management_barrier_programming_mode<NO_BARRIER_DMAS_SCHEDULED>} dmas((%321, %325) : (!VPURegMapped.Index<0:0:0>, !VPURegMapped.Index<0:1:0>)) actKernelRanges(%177, %313 : !VPURegMapped.Index<0:0:0>, !VPURegMapped.Index<1:0:0>) actKernelInvocations(%181, %317 : !VPURegMapped.Index<0:0:0>, !VPURegMapped.Index<1:0:0>) barriers(%42 : !VPURegMapped.Index<0:0:0>) actShaveRt(%327 : !VPURegMapped.Index<0:0:0>) dmaHwpBase(%4 : memref<16xui32, [@CMX_NN, 0]>) dmaCount([[4, 1], [0, 0]]) invariantCount([0, 0]) variantCount([0, 0]) actKernelRangesCount([4, 4]) actKernelInvocationsCount([4, 4]) mediaCount(0) barrierCount(7) finalBarrierId(6) barrierConfigurationTasksCount(224) -> !VPURegMapped.Index<0:0:0>
...
//CHECK:  VPURegMapped.Enqueue previousTaskIdx([[ENQ0]] : !VPURegMapped.Index<0:0:0>) at([[VAL42]] : !VPURegMapped.Index<0:0:0>)
