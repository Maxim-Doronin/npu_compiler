...
func.func @UnrollNceSoHSEPOverlappedThreeClusters() -> !Output_DDR {
    ...
    // CHECK-SAME:      [1, 1, 1025, 2049, 3073, 3073]
    // CHECK-SAME:      [4097, 4097, 5121, 6145, 7169, 7169]
    // CHECK-SAME:      [8193, 8193, 9217, 10241, 11265, 11265]
    // CHECK{LITERAL}:  ]]]> : tensor<1x1x3x6xi32, {order = #NHWC}>

    // CHECK{LITERAL}:  const.Declare memref<1x1x3x6xi32, #NHWC, @DDR> = dense<[[[
    // CHECK-SAME:      [2, 2, 1026, 2050, 3074, 3074]
    // CHECK-SAME:      [4098, 4098, 5122, 6146, 7170, 7170]
    // CHECK-SAME:      [4098, 4098, 5122, 6146, 7170, 7170]
    // CHECK{LITERAL}:  ]]]> : tensor<1x1x3x6xi32, {order = #NHWC}>
}
...
func.func @UnrollNceSoWSEPOverlappedThreeClusters() -> !Output_DDR {
    ...
    // CHECK-SAME:      [1, 1025, 2049]
    // CHECK-SAME:      [1, 1025, 2049]
    // CHECK-SAME:      [4097, 5121, 6145]
    // CHECK-SAME:      [8193, 9217, 10241]
    // CHECK-SAME:      [12289, 13313, 14337]
    // CHECK-SAME:      [12289, 13313, 14337]
    // CHECK{LITERAL}:  ]]]> : tensor<1x1x6x3xi32, {order = #NHWC}>

    // CHECK{LITERAL}:  const.Declare memref<1x1x6x3xi32, #NHWC, @DDR> = dense<[[[
    // CHECK-SAME:      [2, 1026, 1026]
    // CHECK-SAME:      [2, 1026, 1026]
    // CHECK-SAME:      [4098, 5122, 5122]
    // CHECK-SAME:      [8194, 9218, 9218]
    // CHECK-SAME:      [12290, 13314, 13314]
    // CHECK-SAME:      [12290, 13314, 13314]
    // CHECK{LITERAL}:  ]]]> : tensor<1x1x6x3xi32, {order = #NHWC}>
}
