...
func.func @UnrollSwKernel()
    ...
    // CHECK:           VPUIP.SW.Kernel {resultSegmentSizes = array<i32: 1, 0, 0>} @VPU.SW::@builtin_MVN inputs([[TILE0]] as [[ARG_0:%[^:]+]]: memref<1x64x64x32xf16, #NWHC, [@CMX_NN, 0]>) outputs([[OUTPUT0]] as [[ARG_1:%[^:]+]]: memref<1x64x64x32xf16, #NWHC, [@CMX_NN, 0]>) on tile 0 list 0 -> memref<1x64x64x32xf16, #NWHC, [@CMX_NN, 0]>{
    ...
    // CHECK:           VPUIP.SW.Kernel {resultSegmentSizes = array<i32: 1, 0, 0>} @VPU.SW::@builtin_MVN inputs([[TILE1]] as [[ARG_0:%[^:]+]]: memref<1x64x64x32xf16, #NWHC, [@CMX_NN, 0]>) outputs([[OUTPUT1]] as [[ARG_1:%[^:]+]]: memref<1x64x64x32xf16, #NWHC, [@CMX_NN, 0]>) on tile 0 list 1 -> memref<1x64x64x32xf16, #NWHC, [@CMX_NN, 0]>{
    // CHECK:                    VPUIP.SW.Kernel.run {attrs = [false, true, 6.0892105102539063E-4]}([[ARG_0]], [[ARG_1]]) : memref<1x64x64x32xf16, #NWHC, [@CMX_NN, 0]>, memref<1x64x64x32xf16, #NWHC, [@CMX_NN, 0]>
    // CHECK:           }
    // CHECK:   }
