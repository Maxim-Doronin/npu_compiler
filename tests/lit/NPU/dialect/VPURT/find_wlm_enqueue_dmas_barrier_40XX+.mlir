...
// RUN: vpux-opt --split-input-file --init-compiler="vpu-arch=%arch%" --find-wlm-enqueue-dmas-barrier %s | FileCheck %s
...
func.func @DmaAndDpuGraph() -> memref<1x16x8x32xf16,  #NHWC, [@CMX_NN, 0]> {
    ...
    // CHECK-NEXT: VPUIP.NNDMA {port = 0 : i64}
    // CHECK:   VPURT.Task waits([[BAR5]] : !VPURT.Barrier) updates([[BAR6]] : !VPURT.Barrier) enqueueTarget([[BAR3]] : !VPURT.Barrier) wlmPage(2) {
    // CHECK-NEXT: VPUIP.NCEClusterTask
    // CHECK:   VPURT.Task waits([[BAR6]] : !VPURT.Barrier) updates([[BAR7]] : !VPURT.Barrier) enqueueTarget([[BAR3]] : !VPURT.Barrier) wlmPage(3) {
    // CHECK-NEXT: VPUIP.NCEClusterTask
