...
func.func @DepthToSpaceDynamicHeightAndWidth(%arg0: tensor<1x144x?x?xf16, {bounds = #const.OpaqueI64Elements<[1, 144, 3, 3]> : tensor<4xsi64>, order = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>}>) -> tensor<1x16x?x?xf16, {bounds = #const.OpaqueI64Elements<[1, 16, 9, 9]> : tensor<4xsi64>, order = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>}> {
    ...
    //CHECK-DAG: [[CST:%.+]] = const.Declare tensor<16x144x3x3xf16>
    //CHECK-DAG: [[CST_0:%.+]] = const.Declare tensor<1x1x1x1xf16> = dense<0.000000e+00>
    //CHECK-DAG: [[CST_1:%.+]] = const.Declare tensor<1x1x1x1xf16> = dense<1.000000e+00>
    //CHECK: [[FQ0:%.+]] = IE.FakeQuantize([[CST]], [[CST_0]], [[CST_1]], [[CST_0]], [[CST_1]]) {auto_broadcast = #IE.auto_broadcast_type<NUMPY>, levels = 2 : i64} : tensor<16x144x3x3xf16>, tensor<1x1x1x1xf16>, tensor<1x1x1x1xf16>, tensor<1x1x1x1xf16>, tensor<1x1x1x1xf16> -> tensor<16x144x3x3xf16>
    //CHECK: [[VAL1:%.+]] = IE.TransposedConvolution([[ARG0]], [[FQ0]]) {dilations = [1, 1], operandSegmentSizes = array<i32: 1, 1, 0, 0>, pads_begin = [0, 0], pads_end = [0, 0], spatial_output_padding = [0, 0], strides = [3, 3]} : tensor<1x144x?x?xf16, {bounds = #const.OpaqueI64Elements<[1, 144, 3, 3]> : tensor<4xsi64>, order = #NHWC}>, tensor<16x144x3x3xf16> -> tensor<1x16x?x?xf16, {bounds = #const.OpaqueI64Elements<[1, 16, 9, 9]> : tensor<4xsi64>, order = #NHWC}>
