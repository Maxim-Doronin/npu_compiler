...
// CHECK-SAME:    [[ARG_0:%[^:]+]]: tensor<24x16x7x8xf32>
  func.func @ConvertConstToAttr(%arg0: tensor<24x16x7x8xf32>) -> tensor<1x32x20x31xf32> {
    ...
    // CHECK: [[VAL0:%.+]] = IE.BatchToSpace([[ARG_0]]) {block_shape_value = [1, 2, 3, 4], crops_begin_value = [0, 0, 0, 1], crops_end_value = [0, 0, 1, 0], operandSegmentSizes = array<i32: 1, 0, 0, 0>} : tensor<24x16x7x8xf32> -> tensor<1x32x20x31xf32>
