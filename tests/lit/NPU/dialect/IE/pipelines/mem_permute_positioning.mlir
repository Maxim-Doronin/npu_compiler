...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --mempermute-positioning %s | FileCheck %s
...
func.func @MemPemutePositioning(%arg0: tensor<1x16x32x64xf16>) -> tensor<1x16x64x32xi8> {
    ...
    // CHECK:   [[POOL:%.+]] = IE.MaxPool([[MEMPERMUTE0]]) {clamp = {max = 6.000000e+00 : f64, min = 0.000000e+00 : f64}, kernel_size = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], rounding_type = #IE.rounding_type<FLOOR>, strides = [1, 1]} : tensor<1x16x32x64xf16, {order = #NHWC}> -> tensor<1x16x32x64xf16, {order = #NHWC}>
