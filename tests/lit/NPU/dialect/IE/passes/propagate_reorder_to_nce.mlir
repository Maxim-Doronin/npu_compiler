...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --propagate-reorder-to-nce %s | FileCheck %s
...
// CHECK-SAME:      [[ARG_0:%[^:]+]]: tensor<1x3x512x512xf16, {order = #NHWC}>
...
func.func @NotPropagateForMultiUsers(%arg0: tensor<1x3x512x512xf16, {order = #NHWC}>) -> (tensor<1x3x512x512xf16>, tensor<1x3x512x512xf16, {order = #NHWC}>) {
    ...
    //CHECK:            [[CONV:%.+]] = IE.Convolution([[ARG_0]], [[WEIGHT]]) {dilations = [1, 1], pads_begin = [1, 1], pads_end = [1, 1], strides = [1, 1]} : tensor<1x3x512x512xf16, {order = #NHWC}>, tensor<3x3x3x3xf16, {order = #NHWC}> -> tensor<1x3x512x512xf16, {order = #NHWC}>
