...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform% compilation-mode=DefaultHW" --handle-large-strides --canonicalize %s | FileCheck %s
// REQUIRES: platform-NPU3720 || platform-NPU4000 || platform-NPU5010


// CHECK-LABEL: @HandleLargeStridesPrimeStride
// CHECK-SAME:      [[ARG_0:%[^:]+]]: tensor<1x16x28x28xf16>
func.func @HandleLargeStridesPrimeStride(%arg0: tensor<1x16x28x28xf16>) -> tensor<1x32x3x3xf16> {
  ...
  // CHECK:       [[SLICED_INPUT0:%.+]] = IE.Slice [[ARG_0]] [0, 0, 0, 0] [1, 16, 3, 3] : tensor<1x16x28x28xf16> to tensor<1x16x3x3xf16>

  // CHECK:       [[SLICED_CONV0:%.+]] = IE.Convolution([[SLICED_INPUT0]], [[CST]])
  // CHECK-SAME:  {dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]}

  // CHECK:       [[SLICED_INPUT1:%.+]] = IE.Slice [[ARG_0]] [0, 0, 0, 11] [1, 16, 3, 3] : tensor<1x16x28x28xf16> to tensor<1x16x3x3xf16>

  // CHECK:       [[SLICED_CONV1:%.+]] = IE.Convolution([[SLICED_INPUT1]], [[CST]])
  // CHECK-SAME:  {dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]}

  // CHECK:       [[SLICED_INPUT2:%.+]] = IE.Slice [[ARG_0]] [0, 0, 0, 22] [1, 16, 3, 3] : tensor<1x16x28x28xf16> to tensor<1x16x3x3xf16>
  ...
  // CHECK:       [[SLICED_INPUT3:%.+]] = IE.Slice [[ARG_0]] [0, 0, 11, 0] [1, 16, 3, 3] : tensor<1x16x28x28xf16> to tensor<1x16x3x3xf16>

  // CHECK:       [[SLICED_CONV3:%.+]] = IE.Convolution([[SLICED_INPUT3]], [[CST]])
  // CHECK-SAME:  {dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]}

  // CHECK:       [[SLICED_INPUT4:%.+]] = IE.Slice [[ARG_0]] [0, 0, 11, 11] [1, 16, 3, 3] : tensor<1x16x28x28xf16> to tensor<1x16x3x3xf16>

  // CHECK:       [[SLICED_CONV4:%.+]] = IE.Convolution([[SLICED_INPUT4]], [[CST]])
  // CHECK-SAME:  {dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]}

  // CHECK:       [[SLICED_INPUT5:%.+]] = IE.Slice [[ARG_0]] [0, 0, 11, 22] [1, 16, 3, 3] : tensor<1x16x28x28xf16> to tensor<1x16x3x3xf16>
  ...
  // CHECK:       [[SLICED_INPUT6:%.+]] = IE.Slice [[ARG_0]] [0, 0, 22, 0] [1, 16, 3, 3] : tensor<1x16x28x28xf16> to tensor<1x16x3x3xf16>

  // CHECK:       [[SLICED_CONV6:%.+]] = IE.Convolution([[SLICED_INPUT6]], [[CST]])
  // CHECK-SAME:  {dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]}

  // CHECK:       [[SLICED_INPUT7:%.+]] = IE.Slice [[ARG_0]] [0, 0, 22, 11] [1, 16, 3, 3] : tensor<1x16x28x28xf16> to tensor<1x16x3x3xf16>

  // CHECK:       [[SLICED_CONV7:%.+]] = IE.Convolution([[SLICED_INPUT7]], [[CST]])
  // CHECK-SAME:  {dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]}

  // CHECK:       [[SLICED_INPUT8:%.+]] = IE.Slice [[ARG_0]] [0, 0, 22, 22] [1, 16, 3, 3] : tensor<1x16x28x28xf16> to tensor<1x16x3x3xf16>
...
func.func @HandleLargeStridesNonPrimeStride(%arg0: tensor<1x16x28x28xf16>) -> tensor<1x32x2x2xf16> {
  ...
  // CHECK-DAG:       [[CST:%.+]] = const.Declare
  // CHECK:       [[CONV0:%.+]] = IE.Convolution([[ARG_0]], [[CST]])
...
// CHECK-SAME:      [[ARG_0:%[^:]+]]: tensor<1x1x1x2176xf16>
// CHECK-SAME:      [[ARG_1:%[^:]+]]: tensor<258x1x1x256xf16>
func.func @HandleLargeStridesConvolution(%arg0: tensor<1x1x1x2176xf16>, %arg1: tensor<258x1x1x256xf16>) -> tensor<1x2x129x16xf16> {
  ...
  // CHECK:       [[CONV0:%.+]] = IE.Convolution([[ARG_0]], [[ARG_1]])
...
// CHECK-SAME:      [[ARG_0:%[^:]+]]: tensor<1x32x22x22xf16>
func.func @HandleLargeStridesAvgPoolWithFQinput(%arg0: tensor<1x32x22x22xf16>) -> tensor<1x32x2x2xf16> {
    ...
    // CHECK-DAG: [[CST:%.+]] = const.Declare tensor<1x1x1x1xf16> = dense<40.2778358> : tensor<1x1x1x1xf32>, [#const.CastElemType<f16>]
    // CHECK-DAG: [[CST_0:%.+]] = const.Declare tensor<1x1x1x1xf16> = dense<-40.594986> : tensor<1x1x1x1xf32>, [#const.CastElemType<f16>]
    // CHECK: [[FQ0:%.+]] = IE.FakeQuantize([[ARG_0]], [[CST_0]], [[CST]], [[CST_0]], [[CST]]) {auto_broadcast = #IE.auto_broadcast_type<NUMPY>, levels = 256 : i64} : tensor<1x32x22x22xf16>, tensor<1x1x1x1xf16>, tensor<1x1x1x1xf16>, tensor<1x1x1x1xf16>, tensor<1x1x1x1xf16> -> tensor<1x32x22x22xf16>
...
// CHECK-SAME:      [[ARG_0:%[^:]+]]: tensor<1x16x24x42xf16>
func.func @HandleLargeStridesAvgPoolMpThenGeneral(%arg0: tensor<1x16x24x42xf16>) -> tensor<1x16x2x2xf16> {
    ...
    // CHECK:       [[AVGPOOL:%.+]] = IE.AvgPool([[ARG_0]]) {kernel_size = [12, 22], pads_begin = [0, 1], pads_end = [0, 1],
