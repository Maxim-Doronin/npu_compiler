...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --convert-strided-slice-to-conv %s | FileCheck %s
...
func.func @ConvertStridedSlice2ConvOnH(%arg0: tensor<1x3x640x640xf16, {order = #NHWC}>) -> tensor<1x3x320x640xf16, {order = #NHWC}> {
    ...
    // CHECK: [[CONV:%.+]] = IE.Convolution([[ARG_0]], [[CST]]) {dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [2, 1]} : tensor<1x3x640x640xf16, {order = #NHWC}>, tensor<3x3x1x1xf16> -> tensor<1x3x320x640xf16, {order = #NHWC}>
...
// CHECK-SAME:     [[ARG_0:%[^:]+]]: tensor<1x3x320x640xf16, {order = #NHWC}>
func.func @ConvertForUnalignedChannels(%arg0: tensor<1x3x320x640xf16, {order = #NHWC}>) -> tensor<1x3x320x320xf16, {order = #NHWC}> {
    ...
    // CHECK-SAME{LITERAL}: [[[0.000000e+00]], [[1.000000e+00]], [[0.000000e+00]]],
    // CHECK-SAME{LITERAL}: [[[0.000000e+00]], [[0.000000e+00]], [[1.000000e+00]]]]> : tensor<3x3x1x1xf32>, [#const.CastElemType<f16>]
    // CHECK: [[SLICE:%.+]] = IE.Slice [[ARG_0]] [0, 0, 0, 1] [1, 3, 320, 639] : tensor<1x3x320x640xf16, {order = #NHWC}> to tensor<1x3x320x639xf16, {order = #NHWC}>
...
func.func @ConvertStridedSlice2ConvWithStridesOnWH(%arg0: tensor<1x3x416x416xf16>) -> tensor<1x3x208x208xf16> {
    ...
    // CHECK-SAME{LITERAL}: [[[0.000000e+00]], [[1.000000e+00]], [[0.000000e+00]]],
    // CHECK-SAME{LITERAL}: [[[0.000000e+00]], [[0.000000e+00]], [[1.000000e+00]]]]> : tensor<3x3x1x1xf32>, [#const.CastElemType<f16>]
    // CHECK: [[CONV_0:%.+]] = IE.Convolution([[ARG_0]], [[CST0]]) {dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [2, 2]} : tensor<1x3x416x416xf16>, tensor<3x3x1x1xf16> -> tensor<1x3x208x208xf16>
    // CHECK: [[SLICE:%.+]] = IE.Slice [[ARG_0]] [0, 0, 1, 0] [1, 3, 415, 416] : tensor<1x3x416x416xf16> to tensor<1x3x415x416xf16>
...
func.func @ConvertStridedSliceWithFQ2Conv(%arg0: tensor<1x3x640x640xf16, {order = #NHWC}>) -> tensor<1x3x320x640xf16, {order = #NHWC}> {
    ...
    // CHECK: [[FQ_0:%.+]] = IE.FakeQuantize([[ARG_0]], [[CST3]], [[CST2]], [[CST3]], [[CST2]]) {auto_broadcast = #IE.auto_broadcast_type<NUMPY>, levels = 256 : i64} : tensor<1x3x640x640xf16, {order = #NHWC}>, tensor<1x1x1x1xf16>, tensor<1x1x1x1xf16>, tensor<1x1x1x1xf16>, tensor<1x1x1x1xf16> -> tensor<1x3x640x640xf16, {order = #NHWC}>
...
// CHECK-SAME:     [[ARG_0:%[^:]+]]: tensor<1x3x640x640xsi32, {order = #NHWC}>
func.func @NotConvertStridedSliceIfInputTypeSI32(%arg0: tensor<1x3x640x640xsi32, {order = #NHWC}>) -> tensor<1x3x320x640xsi32, {order = #NHWC}> {
    ...
    // CHECK: [[SLICE:%.+]] = IE.StridedSlice([[ARG_0]]) {begin_mask = [0, 0, 0, 0], begins_attr = [0, 0, 0, 0], ellipsis_mask = [0, 0, 0, 0], end_mask = [0, 0, 0, 0], ends_attr = [1, 3, 640, 640], new_axis_mask = [0, 0, 0, 0], operandSegmentSizes = array<i32: 1, 0, 0, 0>, shrink_axis_mask = [0, 0, 0, 0], strides_attr = [1, 1, 2, 1]} : tensor<1x3x640x640xsi32, {order = #NHWC}> -> tensor<1x3x320x640xsi32, {order = #NHWC}>
...
func.func @ConvertParallelStridedSlicesToConv(%arg0: tensor<1x3x416x416xf16>) -> tensor<1x12x208x208xf16> {
    ...
    // CHECK: [[CONV:%.+]] = IE.Convolution([[ARG_0]], [[CST]]) {dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [2, 2]} : tensor<1x3x416x416xf16>, tensor<12x3x2x2xf16> -> tensor<1x12x208x208xf16>
...
func.func @ConvertParallelStridedSlicesToConvWithFQ(%arg0: tensor<1x3x416x416xf16>) -> tensor<1x12x208x208xf16> {
    ...
    // CHECK: [[FQ_0:%.+]] = IE.FakeQuantize([[ARG_0]], [[CST_3]], [[CST_2]], [[CST_3]], [[CST_2]]) {auto_broadcast = #IE.auto_broadcast_type<NUMPY>, levels = 256 : i64} : tensor<1x3x416x416xf16>, tensor<1x1x1x1xf16>, tensor<1x1x1x1xf16>, tensor<1x1x1x1xf16>, tensor<1x1x1x1xf16> -> tensor<1x3x416x416xf16>
...
func.func @NotConvertParallelStridedSlicesToConv(%arg0: tensor<1x3x416x416xf16>) -> tensor<1x3x208x832xf16> {
    ...
    // CHECK-SAME{LITERAL}: [[[0.000000e+00]], [[1.000000e+00]], [[0.000000e+00]]],
    // CHECK-SAME{LITERAL}: [[[0.000000e+00]], [[0.000000e+00]], [[1.000000e+00]]]]> : tensor<3x3x1x1xf32>, [#const.CastElemType<f16>]
    // CHECK: [[CONV_0:%.+]] = IE.Convolution([[ARG_0]], [[CST]]) {dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [2, 2]} : tensor<1x3x416x416xf16>, tensor<3x3x1x1xf16> -> tensor<1x3x208x208xf16>
    // CHECK: [[SLICE_1:%.+]] = IE.Slice [[ARG_0]] [0, 0, 1, 0] [1, 3, 415, 416] : tensor<1x3x416x416xf16> to tensor<1x3x415x416xf16>
    ...
    // CHECK: [[SLICE_2:%.+]] = IE.Slice [[ARG_0]] [0, 0, 0, 1] [1, 3, 416, 415] : tensor<1x3x416x416xf16> to tensor<1x3x416x415xf16>
    ...
    // CHECK: [[SLICE_3:%.+]] = IE.Slice [[ARG_0]] [0, 0, 1, 1] [1, 3, 415, 415] : tensor<1x3x416x416xf16> to tensor<1x3x415x415xf16>
