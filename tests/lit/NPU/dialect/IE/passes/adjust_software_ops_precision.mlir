...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --adjust-software-ops-precision --canonicalize %s | FileCheck %s
...
// CHECK-SAME:    ([[ARG_0:%[^:]+]]: tensor<1x77xsi64>)
func.func @TopK_SI64toFP16(%arg0: tensor<1x77xsi64>) -> (tensor<1x1xsi64>, tensor<1x1xsi32>) {
    ...
    // CHECK: [[INPUT_CVT:%.+]] = IE.Convert([[ARG_0]]) {dstElemType = f16} : tensor<1x77xsi64> -> tensor<1x77xf16>
