...
// CHECK-SAME: [[ARG_0:%[^:]+]]: tensor<2x16xf32>
func.func @UnrollFullyConnectedBatch(%arg0: tensor<2x16xf32>) -> tensor<2x64xf32> {
    ...
    // CHECK:       [[INPUT:%.+]] = IE.Convert([[ARG_0]]) {dstElemType = f16} : tensor<2x16xf32> -> tensor<2x16xf16>
...
// CHECK-SAME: [[ARG_0:%[^:]+]]: tensor<2x128x40x8xf16>
func.func @UnrollEltwiseAddBatch(%arg0: tensor<2x128x40x8xf16>) -> tensor<2x128x40x8xf16> {
    ...
    // CHECK: [[SLICE0_ARG0:%.+]] = IE.Slice [[ARG_0]] [0, 0, 0, 0] [1, 128, 40, 8] : tensor<2x128x40x8xf16> to tensor<1x128x40x8xf16>
    ...
    // CHECK: [[SLICE1_ARG0:%.+]] = IE.Slice [[ARG_0]] [1, 0, 0, 0] [1, 128, 40, 8] : tensor<2x128x40x8xf16> to tensor<1x128x40x8xf16>
...
// CHECK-LABEL: @UnrollSigmoidBatch
// CHECK-SAME: [[ARG_0:%[^:]+]]: tensor<3x9x16x1xf16>
func.func @UnrollSigmoidBatch(%arg0: tensor<3x9x16x1xf16>) -> tensor<3x9x16x1xf16> {
    ...
    // CHECK: [[SLICE0_ARG0:%.+]] = IE.Slice [[ARG_0]] [0, 0, 0, 0] [1, 9, 16, 1] : tensor<3x9x16x1xf16> to tensor<1x9x16x1xf16>
    // CHECK: [[SIGMOID0:%.+]] = IE.Sigmoid([[SLICE0_ARG0]]) :
    // CHECK-SAME: tensor<1x9x16x1xf16> -> tensor<1x9x16x1xf16>
    // CHECK: [[SLICE1_ARG0:%.+]] = IE.Slice [[ARG_0]] [1, 0, 0, 0] [1, 9, 16, 1] : tensor<3x9x16x1xf16> to tensor<1x9x16x1xf16>
    // CHECK: [[SIGMOID1:%.+]] = IE.Sigmoid([[SLICE1_ARG0]]) :
    // CHECK-SAME: tensor<1x9x16x1xf16> -> tensor<1x9x16x1xf16>
    // CHECK: [[SLICE2_ARG0:%.+]] = IE.Slice [[ARG_0]] [2, 0, 0, 0] [1, 9, 16, 1] : tensor<3x9x16x1xf16> to tensor<1x9x16x1xf16>
...
// CHECK-LABEL: @UnrollGroupConvolutionBatch
// CHECK-SAME: [[ARG_0:%[^:]+]]: tensor<3x9x16x1xf16>
// CHECK-SAME: [[ARG_1:%[^:]+]]: tensor<9x1x1x1xf16>
func.func @UnrollGroupConvolutionBatch(%arg0: tensor<3x9x16x1xf16>, %arg1: tensor<9x1x1x1xf16>) -> tensor<3x9x16x1xf16> {
    ...
    return %0 : tensor<3x9x16x1xf16>
    // CHECK: [[SLICE0_ARG0:%.+]] = IE.Slice [[ARG_0]] [0, 0, 0, 0] [1, 9, 16, 1] : tensor<3x9x16x1xf16> to tensor<1x9x16x1xf16>
    // CHECK: [[GROUPCONVOLUTION0:%.+]] = IE.GroupConvolution([[SLICE0_ARG0]], [[ARG_1]]) {dilations = [1, 1], groups = 9 : i64, pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]} :
    // CHECK-SAME: tensor<1x9x16x1xf16>, tensor<9x1x1x1xf16> -> tensor<1x9x16x1xf16>
    // CHECK: [[SLICE1_ARG0:%.+]] = IE.Slice [[ARG_0]] [1, 0, 0, 0] [1, 9, 16, 1] : tensor<3x9x16x1xf16> to tensor<1x9x16x1xf16>
    // CHECK: [[GROUPCONVOLUTION1:%.+]] = IE.GroupConvolution([[SLICE1_ARG0]], [[ARG_1]]) {dilations = [1, 1], groups = 9 : i64, pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]} :
    // CHECK-SAME: tensor<1x9x16x1xf16>, tensor<9x1x1x1xf16> -> tensor<1x9x16x1xf16>
    // CHECK: [[SLICE2_ARG0:%.+]] = IE.Slice [[ARG_0]] [2, 0, 0, 0] [1, 9, 16, 1] : tensor<3x9x16x1xf16> to tensor<1x9x16x1xf16>
    // CHECK: [[GROUPCONVOLUTION2:%.+]] = IE.GroupConvolution([[SLICE2_ARG0]], [[ARG_1]]) {dilations = [1, 1], groups = 9 : i64, pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]} :
    // CHECK-SAME: tensor<1x9x16x1xf16>, tensor<9x1x1x1xf16> -> tensor<1x9x16x1xf16>
...
// CHECK-LABEL: @UnrollMaxPoolingBatch
// CHECK-SAME: [[ARG_0:%[^:]+]]: tensor<2x128x32x64xf16>
func.func @UnrollMaxPoolingBatch(%arg0: tensor<2x128x32x64xf16>) -> tensor<2x128x32x64xf16> {
    ...
    // CHECK:   [[SLICE_0:%.+]] = IE.Slice [[ARG_0]] [0, 0, 0, 0] [1, 128, 32, 64] :
    ...
    // CHECK:   [[SLICE_1:%.+]] = IE.Slice [[ARG_0]] [1, 0, 0, 0] [1, 128, 32, 64] :
    // CHECK-SAME:      tensor<2x128x32x64xf16> to tensor<1x128x32x64xf16>
...
func.func @NotUnrollMemPermuteIfCannotConvertToPolling(%arg0: tensor<2x2x124x124xf16, {order = #NHWC}>) -> tensor<4x1x124x124xf16> {
    ...
    // CHECK:       [[RESHAPE_IN:%.+]] = IE.ShapeCast {shape = [4, 1, 124, 124]} inputs([[INPUT]] : tensor<2x2x124x124xf16, {order = #NHWC}>) -> tensor<4x1x124x124xf16, {order = #NHWC}>
...
func.func @UnrollMultiply(%input1 : tensor<2x3x576x576xf16>, %input2 : tensor<2x1x576x576xf16>) -> tensor<2x3x576x576xf16> {
  ...
  return %res : tensor<2x3x576x576xf16>
    ...
    // CHECK: [[CONCAT:%.+]] = IE.Concat([[MUL1]], [[MUL2]]) {
    // CHECK-SAME:      per_axis = #IE.Concat<axis = 0 : i64>
