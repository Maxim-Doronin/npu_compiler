...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --adjust-groupconv-shape %s | FileCheck %s
// REQUIRES: platform-NPU3720 || platform-NPU4000 || platform-NPU5010

#NHWC = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>
// CHECK-LABEL: @adjustGroupConvInput
// CHECK-SAME:    ([[ARG_0:%[^:]+]]: tensor<1x2x64x512xf16, {order = #NHWC}>)
func.func @adjustGroupConvInput(%arg0: tensor<1x2x64x512xf16, {order = #NHWC}>) -> tensor<1x2x64x512xf16, {order = #NHWC}> {
  ...
  // CHECK:   [[INPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 64, 64]} inputs([[ARG_0]] : tensor<1x2x64x512xf16, {order = #NHWC}>) -> tensor<1x16x64x64xf16, {order = #NHWC}>
  ...
  // CHECK:   [[RESULT:%.+]] = IE.ShapeCast {shape = [1, 2, 64, 512]}  inputs([[CONV_RET]] : tensor<1x16x64x64xf16, {order = #NHWC}>) -> tensor<1x2x64x512xf16, {order = #NHWC}>
  // CHECK:   return [[RESULT]]
}
...
func.func @adjustGroupConvInputFailForCSTShapeNotAlign(%arg0: tensor<1x2x64x512xf16, {order = #NHWC}>) -> tensor<1x2x64x512xf16, {order = #NHWC}> {
  ...
  // CHECK:       [[RESULT:%.+]] = IE.GroupConvolution([[ARG_0]], [[CST_WEIGHTS]], [[CST_BIAS]]) {dilations = [1, 1], groups = 2 : i64, pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]}
  // CHECK:       return [[RESULT]]
}
...
// CHECK-SAME:    ([[ARG_0:%[^:]+]]: tensor<1x3x64x52xf16, {order = #NHWC}>)
func.func @adjustGroupConvInputForDimCFromDimHW(%arg0: tensor<1x3x64x52xf16, {order = #NHWC}>) -> tensor<1x3x64x52xf16, {order = #NHWC}> {
  ...
  // CHECK:   [[INPUT:%.+]] = IE.ShapeCast {shape = [1, 48, 16, 13]} inputs([[ARG_0]] : tensor<1x3x64x52xf16, {order = #NHWC}>) -> tensor<1x48x16x13xf16, {order = #NHWC}>
  ...
  // CHECK:   [[RESULT:%.+]] = IE.ShapeCast {shape = [1, 3, 64, 52]} inputs([[CONV_RET]]
  // CHECK:   return [[RESULT]]
}
...
func.func @adjustGroupConvInputForElem(%arg0: tensor<1x3x64x52xf16, {order = #NHWC}>) -> tensor<1x3x64x52xf16, {order = #NHWC}> {
  ...
  // CHECK:   [[INPUT:%.+]] = IE.ShapeCast {shape = [1, 48, 16, 13]} inputs([[ARG0]]
...
func.func @adjustGroupConvInputForElemAndReshapeCst(%arg0: tensor<1x17x64x52xf16, {order = #NHWC}>) -> tensor<1x17x64x52xf16, {order = #NHWC}> {
  ...
  // CHECK:   [[INPUT:%.+]] = IE.ShapeCast {shape = [1, 272, 16, 13]} inputs([[ARG0]]
  ...
  // CHECK:   [[RESULT:%.+]] = IE.ShapeCast {shape = [1, 17, 64, 52]} inputs([[CONV_RET]]
  // CHECK:   return [[RESULT]]
}
...
func.func @adjustGroupConvInputForElemAndConvertReshapeCst(%arg0: tensor<1x49x120x192xf16, {order = #NHWC}>) -> tensor<1x49x120x192xf16, {order = #NHWC}> {
  ...
  // CHECK:   [[ACTIVATION:%.+]] = IE.ShapeCast {shape = [1, 784, 120, 12]} inputs([[ARG0]]
  ...
  // CHECK:   [[RESULT:%.+]] = IE.ShapeCast {shape = [1, 49, 120, 192]} inputs([[GROUPCONV_RET]]
  // CHECK:   return [[RESULT]]
}
...
func.func @adjustGroupConvInputNotAlignForElemAndReshapeCst(%arg0 : tensor<1x1x289x289xf16, {order = #NHWC}>) -> (tensor<1x1x289x289xf16, {order = #NHWC}>) {
    ...
    // CHECK:       [[SHAPECAST0:%.+]] = IE.ShapeCast
    // CHECK-SAME:  {shape = [1, 289, 17, 17]} inputs([[ARG_0]] : tensor<1x1x289x289xf16, {order = #NHWC}>) -> tensor<1x289x17x17xf16, {order = #NHWC}>
    // CHECK:       [[CST:%.+]] = const.Declare
    // CHECK-SAME:  tensor<289x1x1x1xf16, {order = #NHWC}> = dense<1.000000e+00> : tensor<1x1x1x1xf16>, [#const.Reshape<[1, 1, 1, 1]>, #const.Reorder<#NHWC>, #const.Broadcast<0 : i64, 289 : i64>]
    // CHECK:       [[GROUP_CONV:%.+]] = IE.GroupConvolution([[SHAPECAST0]], [[CST]])
    // CHECK-SAME:  {dilations = [1, 1], groups = 289 : i64, pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]}
    // CHECK-SAME:  tensor<1x289x17x17xf16, {order = #NHWC}>, tensor<289x1x1x1xf16, {order = #NHWC}> -> tensor<1x289x17x17xf16, {order = #NHWC}>
    // CHECK:       [[SHAPECAST1:%.+]] = IE.ShapeCast
    // CHECK-SAME:  {shape = [1, 1, 289, 289]} inputs([[GROUP_CONV]] : tensor<1x289x17x17xf16, {order = #NHWC}>) -> tensor<1x1x289x289xf16, {order = #NHWC}>
    // CHECK:       return [[SHAPECAST1]]
}
...
func.func @sliceGroupConvWithReorderOp(%arg0: tensor<1x3x144x478xf16, {order = #NHCW}>) -> tensor<1x3x144x478xf16> {
    ...
    // CHECK:        [[SLICE_INPUT_0:%.+]] = IE.Slice [[INPUT_DATA]] [0, 0, 0, 0] [1, 1, 144, 478]
    // CHECK:        [[PERM_0:%.+]] = IE.PermuteCast([[SLICE_INPUT_0]]) {dst_order = #NHWC, mem_perm = #NCWH} : tensor<1x1x144x478xf16, {order = #NHCW}> -> tensor<1x1x144x478xf16, {order = #NHWC}>
    // CHECK:       [[SHAPE_CAST_0:%.+]] = IE.ShapeCast {shape = [1, 16, 239, 18]} inputs([[PERM_0]] : tensor<1x1x144x478xf16, {order = #NHWC}>) -> tensor<1x16x239x18xf16, {order = #NHWC}>
    // CHECK:        [[CST:%.+]] = const.Declare tensor<16x1x1x1xf16, {order = #NHWC}> = dense<1.000000e+00> : tensor<1x3x1x1x1xf16>, [#const.Reshape<[1, 1, 1, 1]>, #const.Reorder<#NHWC>, #const.Broadcast<0 : i64, 16 : i64>]
    // CHECK:        [[CST_0:%.+]] = const.Declare tensor<1x16x1x1xf16> = dense<
    // CHECK-SAME{LITERAL}:      [[[[0.788085938]], [[1.11230469]], [[2.08007813]]]]
    // CHECK:      > : tensor<1x3x1x1xf32>, [#const.SubView<[0, 0, 0, 0], [1, 1, 1, 1]>, #const.CastElemType<f16>, #const.Broadcast<1 : i64, 16 : i64>, #const.Reshape<[1, 16, 1, 1]>]
    // CHECK:        [[GROUP_CONV_0:%.+]] = IE.GroupConvolution([[SHAPE_CAST_0]], [[CST]], [[CST_0]]) {dilations = [1, 1], groups = 16 : i64, pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]} : tensor<1x16x239x18xf16, {order = #NHWC}>, tensor<16x1x1x1xf16, {order = #NHWC}>, tensor<1x16x1x1xf16> -> tensor<1x16x239x18xf16, {order = #NHWC}>
    // CHECK:       [[SHAPE_CAST_1:%.+]] = IE.ShapeCast {shape = [1, 1, 144, 478]} inputs([[GROUP_CONV_0]] : tensor<1x16x239x18xf16, {order = #NHWC}>) -> tensor<1x1x144x478xf16, {order = #NHWC}>
    // CHECK:        [[SLICE_INPUT_1:%.+]] = IE.Slice [[INPUT_DATA]] [0, 1, 0, 0] [1, 1, 144, 478] : tensor<1x3x144x478xf16, {order = #NHCW}> to tensor<1x1x144x478xf16, {order = #NHCW}>
    // CHECK:        [[PERM_1:%.+]] = IE.PermuteCast([[SLICE_INPUT_1]]) {dst_order = #NHWC, mem_perm = #NCWH} : tensor<1x1x144x478xf16, {order = #NHCW}> -> tensor<1x1x144x478xf16, {order = #NHWC}>
    // CHECK:       [[SHAPE_CAST_2:%.+]] = IE.ShapeCast {shape = [1, 16, 239, 18]} inputs([[PERM_1]] : tensor<1x1x144x478xf16, {order = #NHWC}>) -> tensor<1x16x239x18xf16, {order = #NHWC}>
    // CHECK:        [[CST_1:%.+]] = const.Declare tensor<16x1x1x1xf16, {order = #NHWC}> = dense<1.000000e+00> : tensor<1x3x1x1x1xf16>, [#const.Reshape<[1, 1, 1, 1]>, #const.Reorder<#NHWC>, #const.Broadcast<0 : i64, 16 : i64>]
    // CHECK:        [[CST_2:%.+]] = const.Declare tensor<1x16x1x1xf16> = dense<
    // CHECK-SAME{LITERAL}:      [[[[0.788085938]], [[1.11230469]], [[2.08007813]]]]
    // CHECK-SAME:      > : tensor<1x3x1x1xf32>, [#const.SubView<[0, 1, 0, 0], [1, 1, 1, 1]>, #const.CastElemType<f16>, #const.Broadcast<1 : i64, 16 : i64>, #const.Reshape<[1, 16, 1, 1]>]
    // CHECK:        [[GROUP_CONV_1:%.+]] = IE.GroupConvolution([[SHAPE_CAST_2]], [[CST_1]], [[CST_2]]) {dilations = [1, 1], groups = 16 : i64, pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]} : tensor<1x16x239x18xf16, {order = #NHWC}>, tensor<16x1x1x1xf16, {order = #NHWC}>, tensor<1x16x1x1xf16> -> tensor<1x16x239x18xf16, {order = #NHWC}>
    // CHECK:       [[SHAPE_CAST_3:%.+]] = IE.ShapeCast {shape = [1, 1, 144, 478]} inputs([[GROUP_CONV_1]] : tensor<1x16x239x18xf16, {order = #NHWC}>) -> tensor<1x1x144x478xf16, {order = #NHWC}>
    // CHECK:        [[SLICE_INPUT_2:%.+]] = IE.Slice [[INPUT_DATA]] [0, 2, 0, 0] [1, 1, 144, 478] : tensor<1x3x144x478xf16, {order = #NHCW}> to tensor<1x1x144x478xf16, {order = #NHCW}>
    // CHECK:        [[PERM_2:%.+]] = IE.PermuteCast([[SLICE_INPUT_2]]) {dst_order = #NHWC, mem_perm = #NCWH}
    // CHECK:       [[SHAPE_CAST_4:%.+]] = IE.ShapeCast {shape = [1, 16, 239, 18]} inputs([[PERM_2]] : tensor<1x1x144x478xf16, {order = #NHWC}>) -> tensor<1x16x239x18xf16, {order = #NHWC}>
    // CHECK:        [[CST_3:%.+]] = const.Declare tensor<16x1x1x1xf16, {order = #NHWC}> = dense<1.000000e+00> : tensor<1x3x1x1x1xf16>, [#const.Reshape<[1, 1, 1, 1]>, #const.Reorder<#NHWC>, #const.Broadcast<0 : i64, 16 : i64>]
    // CHECK:        [[CST_4:%.+]] = const.Declare tensor<1x16x1x1xf16> = dense<
    // CHECK-SAME{LITERAL}:      [[[[0.788085938]], [[1.11230469]], [[2.08007813]]]]
    // CHECK-SAME:      > : tensor<1x3x1x1xf32>, [#const.SubView<[0, 2, 0, 0], [1, 1, 1, 1]>, #const.CastElemType<f16>, #const.Broadcast<1 : i64, 16 : i64>, #const.Reshape<[1, 16, 1, 1]>]
    // CHECK:        [[GROUP_CONV_2:%.+]] = IE.GroupConvolution([[SHAPE_CAST_4]], [[CST_3]], [[CST_4]]) {dilations = [1, 1], groups = 16 : i64, pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]} : tensor<1x16x239x18xf16, {order = #NHWC}>, tensor<16x1x1x1xf16, {order = #NHWC}>, tensor<1x16x1x1xf16> -> tensor<1x16x239x18xf16, {order = #NHWC}>
    // CHECK:       [[SHAPE_CAST_5:%.+]] = IE.ShapeCast {shape = [1, 1, 144, 478]} inputs([[GROUP_CONV_2]] : tensor<1x16x239x18xf16, {order = #NHWC}>) -> tensor<1x1x144x478xf16, {order = #NHWC}>
    // CHECK:        [[CONCAT:%.+]] = IE.Concat([[SHAPE_CAST_1]], [[SHAPE_CAST_3]], [[SHAPE_CAST_5]]) {per_axis = #IE.Concat<axis = 1 : i64>} : tensor<1x1x144x478xf16, {order = #NHWC}>, tensor<1x1x144x478xf16, {order = #NHWC}>, tensor<1x1x144x478xf16, {order = #NHWC}> -> tensor<1x3x144x478xf16, {order = #NHWC}>
    // CHECK:        [[REORDER:%.+]] = IE.Reorder([[CONCAT]]) {dstOrder = #NCHW} : tensor<1x3x144x478xf16, {order = #NHWC}> -> tensor<1x3x144x478xf16>
    // CHECK:        return [[REORDER]] : tensor<1x3x144x478xf16>
}
