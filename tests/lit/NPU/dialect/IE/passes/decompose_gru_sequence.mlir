...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --decompose-gru-sequence %s | FileCheck %s
...
// CHECK:           [[VAL_3_CLONE:%.+]] = const.Declare tensor<256x256xf16> = dense<1.000000e+00> : tensor<1x768x256xf16>, [#const.SubView<[0, 512, 0], [1, 256, 256]>, #const.Reshape<[256, 256]>]
...
// CHECK:           [[VAL_40:%.+]] = IE.MatMul([[VAL_39]], [[VAL_3_CLONE]]) {transpose_b} : tensor<2x256xf16>, tensor<256x256xf16> -> tensor<2x256xf16>
...
// CHECK:           [[VAL_2_EXTRA:%.+]] = const.Declare tensor<1xf16> = dense<1.000000e+00> : tensor<1xf16>
// CHECK:           [[VAL_43:%.+]] = IE.Subtract([[VAL_2_EXTRA]], [[VAL_37]]#0) {auto_broadcast = #IE.auto_broadcast_type<NUMPY>} : tensor<1xf16>, tensor<2x256xf16> -> tensor<2x256xf16>
