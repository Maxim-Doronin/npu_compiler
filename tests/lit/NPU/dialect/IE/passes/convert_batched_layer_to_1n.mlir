...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --convert-batched-layer-to-1n %s | FileCheck %s
// REQUIRES: platform-NPU3720 || platform-NPU4000 || platform-NPU5010

#map = affine_map<(d0, d1, d2, d3) -> (d2, d1, d0, d3)>

// CHECK-LABEL: @main
// CHECK-SAME:    [[ARG_0:%[^:]+]]: tensor<5x16x1x1xf16>
// CHECK-SAME:    [[ARG_1:%[^:]+]]: tensor<4x16x1x1xf16>
func.func @main(%arg0: tensor<5x16x1x1xf16>, %arg1: tensor<4x16x1x1xf16>) -> tensor<5x4x1x1xf16> {
    ...
    // CHECK: [[IN_TRANSPOSE:%.+]] = IE.Transpose([[ARG_0]]) {order_value = #map} : tensor<5x16x1x1xf16> -> tensor<1x16x5x1xf16>
    // CHECK: [[CONV:%.+]] = IE.Convolution([[IN_TRANSPOSE]], [[ARG_1]])
...
// CHECK-LABEL: @MixedPrecisionCase
// CHECK-SAME:    [[ARG_0:%[^:]+]]: tensor<5x16x1x1x!qElemType>
// CHECK-SAME:    [[ARG_1:%[^:]+]]: tensor<4x16x1x1x!qElemType1>
func.func @MixedPrecisionCase(%arg0: tensor<5x16x1x1x!qElemType>, %arg1: tensor<4x16x1x1x!qElemType1>) -> tensor<5x4x1x1xf16> {
    ...
    // CHECK: [[IN_TRANSPOSE:%.+]] = IE.Transpose([[ARG_0]]) {order_value = #map} : tensor<5x16x1x1x!qElemType> -> tensor<1x16x5x1x!qElemType>
    // CHECK: [[CONV:%.+]] = IE.Convolution([[IN_TRANSPOSE]], [[ARG_1]])
    // CHECK-SAME:   {dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]} :
...
// CHECK-LABEL: @ConvertWithWNotEq1
// CHECK-SAME:    [[ARG_0:%[^:]+]]: tensor<512x48x1x336xf16>
// CHECK-SAME:    [[ARG_1:%[^:]+]]: tensor<6x48x1x3xf16>
func.func @ConvertWithWNotEq1(%arg0: tensor<512x48x1x336xf16>, %arg1: tensor<6x48x1x3xf16>) -> tensor<512x6x1x336xf16> {
    ...
    // CHECK: [[IN_TRANSPOSE:%.+]] = IE.Transpose([[ARG_0]]) {order_value = #map} : tensor<512x48x1x336xf16> -> tensor<1x48x512x336xf16>
    // CHECK: [[CONV:%.+]] = IE.Convolution([[IN_TRANSPOSE]], [[ARG_1]])
...
// CHECK-LABEL: @NoChagesPerAxisQuantization
// CHECK-SAME:    [[ARG_0:%[^:]+]]: tensor<5x8x1x1x!qElemType>
// CHECK-SAME:    [[ARG_1:%[^:]+]]: tensor<4x8x1x1x!qElemType1>
...
// CHECK-LABEL: @NoChangesFilterPlaneNotEq1
// CHECK-SAME:    [[ARG_0:%[^:]+]]: tensor<5x16x1x1xf16>
// CHECK-SAME:    [[ARG_1:%[^:]+]]: tensor<4x16x2x2xf16>
...
// CHECK-SAME:    [[ARG_0:%[^:]+]]: tensor<5x16x1x1xf16>
// CHECK-SAME:    [[ARG_1:%[^:]+]]: tensor<5x16x1x1xf16>
func.func @ConvertAdd(%arg0: tensor<5x16x1x1xf16>, %arg1: tensor<5x16x1x1xf16>) -> tensor<5x16x1x1xf16> {
    ...
    // CHECK: [[IN_TRANSPOSE1:%.+]] = IE.Transpose([[ARG_0]]) {order_value = #map} : tensor<5x16x1x1xf16> -> tensor<1x16x5x1xf16>
    // CHECK: [[IN_TRANSPOSE2:%.+]] = IE.Transpose([[ARG_1]]) {order_value = #map} : tensor<5x16x1x1xf16> -> tensor<1x16x5x1xf16>
...
func.func @NoChangesAddOp(%arg0: tensor<5x16x1x1xf16>, %arg1: tensor<1x16x1x1xf16>) -> tensor<5x16x1x1xf16> {
    ...
    // CHECK: [[ADD:%.+]] = IE.Add([[ARG_0]], [[ARG_1]]) {auto_broadcast = #IE.auto_broadcast_type<NUMPY>} : tensor<5x16x1x1xf16>, tensor<1x16x1x1xf16> -> tensor<5x16x1x1xf16>
    // CHECK: return [[ADD]] : tensor<5x16x1x1xf16>
}

// -----

// CHECK-LABEL: func.func @ConvertAddWithHW
// CHECK-SAME:    ([[ARG_0:%[^:]+]]: tensor<50x16x10x10xf16>, [[ARG_1:%[^:]+]]: tensor<50x16x10x10xf16>)
...
// CHECK-LABEL: func.func @ConvertAddWithHWWithDiffInputShape
// CHECK-SAME:    ([[ARG_0:%[^:]+]]: tensor<50x16x10x10xf16>, [[ARG_1:%[^:]+]]: tensor<50x16x1x1xf16>)
...
// CHECK-LABEL: @ConvertGroupConvolution
// CHECK-SAME:    ([[ARG_0:%[^:]+]]: tensor<16x16x1x32xf16>)
func.func @ConvertGroupConvolution(%arg0: tensor<16x16x1x32xf16>) -> tensor<16x16x1x32xf16> {
    ...
    // CHECK: [[IN_TRANSPOSE:%.+]] = IE.Transpose([[ARG_0]]) {order_value = #map} : tensor<16x16x1x32xf16> -> tensor<1x16x16x32xf16>
...
// CHECK-LABEL: @NoChangesGroupConvolution
// CHECK-SAME:    ([[ARG_0:%[^:]+]]: tensor<16x16x2x32xf16>)
func.func @NoChangesGroupConvolution(%arg0: tensor<16x16x2x32xf16>) -> tensor<16x16x2x32xf16> {
    ...
    // CHECK: [[CONV:%.+]] = IE.GroupConvolution([[ARG_0]], [[FILTER]], [[BIAS]]) {dilations = [1, 1], groups = 16 : i64, pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]} : tensor<16x16x2x32xf16>, tensor<16x1x1x1xf32>, tensor<1x16x1x1xf32> -> tensor<16x16x2x32xf16>
