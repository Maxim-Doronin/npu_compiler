...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --convert-assign-read-value --mlir-print-debuginfo %s | FileCheck %s
...
func.func @ConvertAssignAndReadValue(%arg0: tensor<1x768xf32>) -> tensor<1x768xf32> {
    ...
    // CHECK:       @ConvertAssignAndReadValue([[ARG_0:%[^:]+]]: tensor<1x768xf32> loc([[LOC_FOO1_ARG0:.+]]), [[ARG_1:%[^:]+]]: tensor<1x768xf32> loc([[LOC_FOO2_ARG0:.+]])) -> (tensor<1x768xf32>, tensor<1x768xf32>)
    // CHECK:       [[VAR0:%.+]] = IE.Add([[ARG_0]], [[ARG_1]]) {auto_broadcast = #IE.auto_broadcast_type<NUMPY>} : tensor<1x768xf32>, tensor<1x768xf32> -> tensor<1x768xf32>
