...
uint32_t getFifoAddr(VPURegMapped::TaskType type, size_t tile, size_t list, int64_t actShavePerTile) {
    VPUX_THROW_WHEN(tile >= VPUMI40XX::NPU_MAX_TILES || list > 1, "Invalid tile index {0} or list index {1}", tile,
                    list);
    ...
    for (uint32_t tileIdx = 0; tileIdx < tilesCount; tileIdx++) {
        for (const auto& [taskType, listCount] : taskTypesWithListCountPerTile) {
            for (uint32_t listIdx = 0; listIdx < listCount; listIdx++) {
                ...
                auto fifoAddr = getFifoAddr(taskType, tileIdx, listIdx, shavesCountPerTile);
