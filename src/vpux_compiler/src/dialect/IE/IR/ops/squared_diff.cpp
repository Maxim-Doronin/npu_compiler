...
mlir::LogicalResult vpux::IE::SquaredDifferenceOp::inferReturnTypeComponents(
    ...
    IE::SquaredDifferenceOpAdaptor squaredDiff(operands, attrs, prop);
    if (mlir::failed(squaredDiff.verify(loc))) {
        return mlir::failure();
    }

    const auto in1Type = mlir::cast<mlir::ShapedType>(squaredDiff.getInput1().getType());
    const auto in2Type = mlir::cast<mlir::ShapedType>(squaredDiff.getInput2().getType());
