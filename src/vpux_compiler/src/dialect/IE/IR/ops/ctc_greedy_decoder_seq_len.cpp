...
mlir::LogicalResult vpux::IE::CTCGreedyDecoderSeqLenOp::inferReturnTypeComponents(
    ...
    IE::CTCGreedyDecoderSeqLenOpAdaptor ctc(operands, attrs, prop);
    if (mlir::failed(ctc.verify(loc))) {
        return mlir::failure();
    }

    const auto inType = mlir::cast<mlir::ShapedType>(ctc.getInput().getType());
    ...
    const auto outElemType = mlir::cast<mlir::ShapedType>(ctc.getSequenceLength().getType()).getElementType();
