...
mlir::LogicalResult vpux::IE::BitwiseOrOp::inferReturnTypeComponents(
    ...
    IE::BitwiseOrOpAdaptor bitwiseOr(operands, attrs, prop);
    if (mlir::failed(bitwiseOr.verify(loc))) {
        return mlir::failure();
    }

    const auto in1Type = mlir::cast<mlir::ShapedType>(bitwiseOr.getInput1().getType());
    const auto in2Type = mlir::cast<mlir::ShapedType>(bitwiseOr.getInput2().getType());
