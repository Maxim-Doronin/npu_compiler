...
mlir::LogicalResult vpux::IE::LogicalOrOp::inferReturnTypeComponents(
    ...
    IE::LogicalOrOpAdaptor logicalor(operands, attrs, prop);
    if (mlir::failed(logicalor.verify(loc))) {
        return mlir::failure();
    }

    const auto in1Type = mlir::cast<mlir::ShapedType>(logicalor.getInput1().getType());
    const auto in2Type = mlir::cast<mlir::ShapedType>(logicalor.getInput2().getType());
