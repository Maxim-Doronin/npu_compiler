...
std::tuple<mlir::Value, Shape, Shape> getInputConcatAndPadding(mlir::PatternRewriter& rewriter, mlir::Operation* origOp,
    ...
    auto createConst = [&](int64_t pad, int64_t kernel, bool isDimX) {
        ...
        auto zeroConst = Const::createZerosConst(rewriter, takeOpLoc(origOp, "pad_zero"), zeroType);
        ...
        return rewriter.createOrFold<IE::ReorderOp>(takeOpLoc(origOp, "reorder"), zeroConst, orderMap);
    };
    ...
    inputConcat =
            rewriter.create<IE::ConcatOp>(takeOpLoc(origOp, "input_concat0"), mlir::ValueRange(concats), Dims4D::Act::W)
                    ->getResult(0);
    ...
    inputConcat =
            rewriter.create<IE::ConcatOp>(takeOpLoc(origOp, "input_concat1"), mlir::ValueRange(concats), Dims4D::Act::H)
                    ->getResult(0);
