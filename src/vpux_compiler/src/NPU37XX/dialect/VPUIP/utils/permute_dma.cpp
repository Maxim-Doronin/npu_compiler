...
mlir::LogicalResult UnrollMultiClusterPermuteDMA::unrollSegmentedOrOverlappedOutput(
        VPUIP::PermuteDMAOp permuteOp, VPUIP::DistributedBufferType distributedType, mlir::AffineMap memPerm,
        mlir::PatternRewriter& rewriter, int64_t portCount, Logger logger) {
    ...
    auto mergedInputShape = VPUIP::getPermuteDMAInputShape(inputType, outputType, memPerm, logger).value();
    auto mergedOutputShape = VPUIP::getPermuteDMAOutputShape(inputType, outputType, memPerm, logger).value();
    auto mergedMemPerm = VPUIP::getPermuteDMAMergedMemPerm(inputType, memPerm);
    auto dmaDescriptorGenerator = VPUIP::PermuteDmaDescriptorGenerator(ctx, mergedMemPerm, logger);
    ...
    auto tileDimForMergedOutput =
            VPUIP::getTileDimForPermuteDMA(inputType, outputType, memPerm, distributedType, logger).value();
    ...
    for (int64_t clusterId = 0; clusterId < numClusters; ++clusterId) {
        auto mergedSubOutputShape =
                VPUIP::getPermuteDMAOutputShape(inTypes[clusterId], outTypes[clusterId], memPerm, logger).value();
    ...
    for (int64_t clusterId = 0; clusterId < numClusters; ++clusterId) {
        ...
        logger.trace("Insert new input buffer declaration: '{0}'", inputBuffer);
        ...
        logger.trace("Insert new output buffer declaration: '{0}'", outBuffer);
        ...
        auto newPermuteDMAOp = VPURT::wrapIntoTaskOp<VPUIP::PermuteDMAOp>(
                ...
                permuteOp.getProfilingMetadataAttr(), /*internalDataFlow=*/nullptr);
...
mlir::LogicalResult UnrollMultiClusterPermuteDMA::unrollDuplicatedOutput(VPUIP::PermuteDMAOp permuteOp,
                                                                         VPUIP::DistributedBufferType distributedType,
                                                                         mlir::AffineMap memPerm,
                                                                         mlir::PatternRewriter& rewriter, int64_t,
                                                                         Logger logger) {
    auto loc = permuteOp->getLoc();
    auto ctx = permuteOp->getContext();
...
mlir::LogicalResult UnrollMultiClusterPermuteDMA::unrollDuplicatedInputAndOutput(VPUIP::PermuteDMAOp permuteOp,
                                                                                 mlir::AffineMap memPerm,
                                                                                 mlir::PatternRewriter& rewriter,
                                                                                 int64_t, Logger logger) {
    auto loc = permuteOp->getLoc();
    auto ctx = permuteOp->getContext();
...
mlir::LogicalResult UnrollMultiClusterPermuteDMA::unrollDuplicatedInput(VPUIP::PermuteDMAOp permuteOp,
                                                                        mlir::AffineMap memPerm,
                                                                        mlir::PatternRewriter& rewriter, int64_t,
                                                                        Logger logger) {
    auto loc = permuteOp->getLoc();
...
mlir::LogicalResult UnrollSingleClusterPermuteDMA::unroll(VPUIP::PermuteDMAOp permuteOp,
    ...
    if (dmaDescriptorAttr == nullptr) {
        auto ctx = permuteOp->getContext();
        auto mergedInputShape = VPUIP::getPermuteDMAInputShape(inType, outType, memPerm, logger).value();
        auto mergedOutputShape = VPUIP::getPermuteDMAOutputShape(inType, outType, memPerm, logger).value();
        auto dmaDescriptorGenerator = VPUIP::PermuteDmaDescriptorGenerator(ctx, mergedMemPerm, logger);
    ...
    auto subInput =
            VPUIP::getPermuteDMASubInputShapes(config::getArch(permuteOp), inType, outType, memPerm, portCount, logger);
    ...
    logger.trace("Unrolling PermuteDMAOp '{0}' at '{1}'", permuteOp->getName(), permuteOp->getLoc());
    ...
    for (auto idx = 0; idx < checked_cast<int64_t>(subInputShapes.size()); idx++) {
        ...
        logger.trace("Create unrolled PermuteDMA operation with input/output shape: {0}/{1}, SrcMemory at {2}, "
                     "DstMemory at {3}",
        ...
        if (firstPermuteDMAsOnPorts.size() < static_cast<size_t>(portCount)) {
