...
class StrategyFactory37XX : public VPUIP::StrategyFactory {
public:
    std::unique_ptr<IIterativeWalkPassStrategy> getUnrollDepthToSpaceDMAStrategy(mlir::MLIRContext* ctx,
        ...
        return std::make_unique<arch37xx::UnrollDepthToSpaceDMAStrategy>(ctx, dmaPortCount);
    }

    std::unique_ptr<IIterativeWalkPassStrategy> getUnrollSpaceToDepthDMAStrategy(mlir::MLIRContext* ctx,
                                                                                 int64_t dmaPortCount) override {
        return std::make_unique<arch37xx::UnrollSpaceToDepthDMAStrategy>(ctx, dmaPortCount);
    }

    std::unique_ptr<IIterativeWalkPassStrategy> getUnrollPermuteDMAStrategy(mlir::MLIRContext* ctx,
                                                                            int64_t dmaPortCount) override {
        return std::make_unique<arch37xx::UnrollPermuteDMAStrategy>(ctx, dmaPortCount);
    }

    std::unique_ptr<IGreedilyPassStrategy> getUnrollPerAxisTileDMAStrategy(int64_t dmaPortCount) override {
        return std::make_unique<arch37xx::UnrollPerAxisTileDMAStrategy>(dmaPortCount);
    }
...
void vpux::VPUIP::StrategiesInitializer37XX::initialize(mlir::MLIRContext* context) {
    auto factory = std::make_unique<VPUIP::StrategyFactory37XX>();
